...
public sealed class ActionPlanRepository(IDbConnectionFactory db) : IActionPlanRepository {
    const string Select = "SELECT p.id,p.title,p.summary,p.origin_type OriginType,p.status,p.priority,p.owner_user_id OwnerUserId,p.due_at DueAt,p.evidence_summary EvidenceSummary,p.expected_outcome ExpectedOutcome,coalesce(round(avg(i.progress_percent))::int,0) ProgressPercent,p.created_at CreatedAt FROM valorapesquisa.action_plans p LEFT JOIN valorapesquisa.action_items i ON i.action_plan_id=p.id AND i.deleted_at IS NULL";
    ...
    public async Task<PageResult<ActionOptionDto>> Options(Guid o, Guid u, bool wide, OptionQuery query, CancellationToken c) {
        ...
        var sql=$"""SELECT count(*)::int FROM valorapesquisa.action_plans p WHERE {scope};
    ...
    public async Task<ActionDashboardDto> Dashboard(Guid o, CancellationToken c) { var plans = await List(o, c); using var x = db.Create(); var items = (await x.QueryAsync<ActionItemDto>(new CommandDefinition(ActionItemRepository.Select + " WHERE organization_id=@o AND deleted_at IS NULL ORDER BY due_at NULLS LAST LIMIT 12", new { o }, cancellationToken: c))).ToList(); return new(items.Count(i => i.Priority == "critical" && i.Status != "completed"), items.Count(i => i.DueAt < DateTime.UtcNow && i.Status is not "completed" and not "canceled"), items.Count(i => i.ResponsibleUserId is null && i.Status is not "completed" and not "canceled"), plans.Count(p => p.Status is "approved" or "in_execution"), items, plans.Take(6).ToList()); }
