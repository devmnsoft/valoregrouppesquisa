...
## 1. Resumo da sprint
A Sprint 02 substituiu os placeholders principais do `projeto .NET predecessor removido` por um fluxo vertical real com PostgreSQL/Dapper: Organização → Usuário → Login → Formulário → Pesquisa → Link público → Resposta pública → Resultado → Auditoria.
...
- `git status --short`
- `sed` nas leituras obrigatórias do projeto .NET predecessor removido e referências legadas.
- `rg` para verificar remoção de placeholders e exposição textual de hashes.
- `dotnet build projeto .NET predecessor removido/solution predecessora removida`

## 11. Comandos não executados e motivo
- `dotnet test projeto .NET predecessor removido/solution predecessora removida` não pôde ser executado porque o container não possui o binário `dotnet` instalado.
