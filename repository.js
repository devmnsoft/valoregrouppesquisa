...
async function read(method,args){const p=selectedProvider();if(!p?.[method])throw new Error(`Provider ${currentDataProvider()==='hybrid'?primaryProvider():currentDataProvider()} não possui método ${method}.`);const result=await p[method](...args);if(shouldUseHybrid()){const s=secondaryProvider();if(s?.[method])Promise.resolve().then(()=>s[method](...args)).then(other=>recordDivergence(method,result,other)).catch(error=>recordDivergence(method,result,{error:error.message}));}return result;}
async function write(method,args){const p=selectedProvider();if(!p?.[method])throw new Error(`Provider ${currentDataProvider()==='hybrid'?primaryProvider():currentDataProvider()} não possui método ${method}.`);return p[method](...args);}
