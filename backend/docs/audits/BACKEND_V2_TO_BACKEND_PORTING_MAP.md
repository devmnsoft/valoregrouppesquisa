# Mapa Técnico — `projeto .NET predecessor removido` → `backend`
...
- **Feito em `projeto .NET predecessor removido`:** fluxo compacto de login/me/menu em controllers e contratos.
...
- **Não portar:** cards estáticos/fake do `projeto .NET predecessor removido`.
