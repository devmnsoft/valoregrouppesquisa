...
public sealed record ExecutivePriorityDto(Guid Id, string Title, string? Description, string Status, string Priority,
    ...
    int ProgressPercent, DateTimeOffset UpdatedAt);
...
public interface IExecutivePriorityRepository {
    ...
    Task<IReadOnlyList<PriorityOptionDto>> OwnersAsync(Guid organizationId, CancellationToken ct);
    Task<IReadOnlyList<PrioritySourceOptionDto>> SourcesAsync(Guid organizationId, Guid userId, bool organizationWide, CancellationToken ct);
}
