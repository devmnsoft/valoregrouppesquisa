...
Foi mantida a solution `backend/Valora.sln`, porque já existe separação compatível com Clean Architecture (`Domain`, `Application`, `Infrastructure`, `Api`, `Web`, `Tests`) e o legado JavaScript/Firebase permanece preservado na raiz. Criar `projeto .NET predecessor removido` nesta etapa aumentaria duplicidade sem ganho técnico imediato.
