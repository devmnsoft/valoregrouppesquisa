...
public sealed class AccessAdministrationRepository(IDbConnectionFactory factory) : IAccessAdministrationRepository
{
    ...
    private static Task Audit(System.Data.IDbConnection c,System.Data.IDbTransaction tx,Guid org,Guid actor,string action,Guid role,IReadOnlyList<string>? before,IReadOnlyList<string>? after,string? reason,CancellationToken ct)=>c.ExecuteAsync(new CommandDefinition("INSERT INTO valorapesquisa.audit_logs(organization_id,actor_id,action,entity_name,entity_id,details,correlation_id) VALUES(@org,@actor,@action,'role',@role,jsonb_build_object('before',@before,'after',@after,'reason',@reason),current_setting('valora.correlation_id',true))",new{org,actor,action,role,before=before?.ToArray(),after=after?.ToArray(),reason},tx,cancellationToken:ct));
