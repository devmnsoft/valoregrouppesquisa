...
window.ValoraFirebaseRepository={
  ...
  loadCompanies({state}={}){return state?.companies||[];},loadOrganizations:loadOrganizations,loadUsers({state}={}){return state?.users||[];},loadPlans({state}={}){return state?.plans||[];},loadModules({state}={}){return state?.modules||[];},loadForms({state}={}){return state?.forms||[];},loadSurveys({state}={}){return state?.surveys||[];},loadResponses({state}={}){return state?.responses||[];},loadInvitations({state}={}){return state?.invitations||[];},loadActionPlans({state}={}){return state?.actionPlans||[];},loadNotifications({state}={}){return state?.notifications||[];},loadInvoices({state}={}){return state?.invoices||[];},loadSettings({state}={}){return state?.settings||{};},
  listOrganizations:loadOrganizations,getOrganization(id){return getDoc('organizations',id);},createOrganization(data){return createDoc('organizations',data);},updateOrganization(id,data){return updateDoc('organizations',id,data);},updateOrganizationBrand(id,brand){return updateDoc('organizations',id,{brand});},updateOrganizationSubscription(id,subscription){return updateDoc('organizations',id,{subscription});},updateOrganizationSettings(id,settings){return updateDoc('organizations',id,{settings});},updateOrganizationLimits(id,limitsOverride){return updateDoc('organizations',id,{limitsOverride});},getOrganizationBySlug(slug){return queryCollection('organizations',[["slug","==",slug]],null,1).then(r=>r[0]||null);},checkSlugAvailability(slug,{currentId=''}={}){return queryCollection('organizations',[["slug","==",slug]],null,1).then(r=>!r.some(x=>x.id!==currentId));},deleteOrganization(id){return deleteDoc('organizations',id);},
  ...
  async saveChanges({state}={}){session.store=state||session.store;if(!session.profile||!session.loaded)return session.store;try{await Promise.all(['companies','users','plans','modules','forms','surveys','invitations','invoices','actionPlans','notifications'].map(syncCollectionFromState));await syncSettings();}catch(err){console.warn('[Valora Pulse] Falha ao salvar no Firestore.',err);throw mapCollectionError(err);}return session.store;},
