...
public sealed class AssistedOperationsRepository(IDbConnectionFactory connections) : IAssistedOperationsRepository {
    ...
    public async Task<IReadOnlyList<IDictionary<string, object?>>> UsageAsync(Guid? organizationId, CancellationToken ct = default) { using var db = connections.Create(); var rows = await db.QueryAsync(new CommandDefinition("SELECT organization_id,period_start,period_end,active_users,logins,diagnostics_created,diagnostics_published,public_links,responses,reports_generated,certificates_generated,actions_created,actions_completed,blocked_features FROM valorapesquisa.usage_analytics_snapshots WHERE (@OrganizationId::uuid IS NULL OR organization_id=@OrganizationId::uuid) ORDER BY period_start DESC LIMIT 100", new { OrganizationId = organizationId }, cancellationToken: ct)); return rows.Select(ToDictionary).ToArray(); }

    public async Task<Guid> RunDataQualityAsync(Guid? userId, string correlationId, CancellationToken ct = default) { using var db = connections.Create(); var id = await db.ExecuteScalarAsync<Guid>(new CommandDefinition("INSERT INTO valorapesquisa.data_quality_runs(status,started_at,created_at,correlation_id,created_by_user_id) VALUES ('completed',now(),now(),@CorrelationId,@UserId) RETURNING id", new { CorrelationId = correlationId, UserId = userId }, cancellationToken: ct)); await db.ExecuteAsync(new CommandDefinition("INSERT INTO valorapesquisa.data_quality_issues(run_id,check_code,entity_type,entity_id,severity,description,status,created_at) SELECT @Id,'organization_without_plan','organization',o.id,'critical','Organização sem plano ativo','open',now() FROM valorapesquisa.organizations o WHERE NOT EXISTS(SELECT 1 FROM valorapesquisa.subscriptions s WHERE s.organization_id=o.id AND s.status='active')", new { Id = id }, cancellationToken: ct)); await Audit(db, null, userId, "data-quality.executed", "data_quality_run", id, correlationId, ct); return id; }

    public async Task<IDictionary<string, object?>> OperationsDashboardAsync(Guid? organizationId, CancellationToken ct = default) {
        using var db = connections.Create();
        const string sql = """SELECT (SELECT count(*)::int FROM valorapesquisa.organizations o WHERE @OrganizationId::uuid IS NULL OR o.id=@OrganizationId::uuid) organizations_active,(SELECT count(*)::int FROM valorapesquisa.onboarding_checklists x WHERE (@OrganizationId::uuid IS NULL OR x.organization_id=@OrganizationId::uuid) AND x.status<>'completed') onboarding_pending,(SELECT count(*)::int FROM valorapesquisa.support_tickets x WHERE (@OrganizationId::uuid IS NULL OR x.organization_id=@OrganizationId::uuid) AND x.status NOT IN ('closed','resolved','cancelled')) tickets_open,(SELECT count(*)::int FROM valorapesquisa.support_tickets x WHERE (@OrganizationId::uuid IS NULL OR x.organization_id=@OrganizationId::uuid) AND x.priority='critical' AND x.status NOT IN ('closed','resolved','cancelled')) tickets_critical,(SELECT count(*)::int FROM valorapesquisa.operational_incidents x WHERE (@OrganizationId::uuid IS NULL OR x.organization_id=@OrganizationId::uuid) AND x.status NOT IN ('closed','resolved')) incidents_open,(SELECT count(*)::int FROM valorapesquisa.customer_feedback x WHERE (@OrganizationId::uuid IS NULL OR x.organization_id=@OrganizationId::uuid) AND x.status='received') feedback_received,(SELECT count(*)::int FROM valorapesquisa.upgrade_requests x WHERE (@OrganizationId::uuid IS NULL OR x.organization_id=@OrganizationId::uuid) AND x.status='requested') upgrades_requested,(SELECT count(*)::int FROM valorapesquisa.data_quality_issues x WHERE (@OrganizationId::uuid IS NULL OR x.organization_id=@OrganizationId::uuid) AND x.severity IN ('high','critical') AND x.status='open') data_quality_critical,(SELECT version FROM valorapesquisa.release_notes WHERE status='published' ORDER BY published_at DESC NULLS LAST LIMIT 1) latest_release""";
