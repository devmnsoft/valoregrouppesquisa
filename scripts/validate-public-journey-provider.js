...
ok(/PUBLIC_SUBMISSION_PROVIDER\s*:\s*['"]external-api['"]/.test(prod),'PUBLIC_SUBMISSION_PROVIDER ausente em produção.');
ok(/PUBLIC_SURVEY_VALIDATION_PROVIDER\s*:\s*['"]external-api['"]/.test(prod),'PUBLIC_SURVEY_VALIDATION_PROVIDER ausente em produção.');
ok(/RESULT_PROVIDER\s*:\s*['"]external-api['"]/.test(prod),'RESULT_PROVIDER ausente em produção.');
...
ok(/ENABLE_CLOUD_FUNCTIONS\s*:\s*false/.test(prod)&&!/PUBLIC_SUBMISSION_PROVIDER\s*:\s*['"]cloud-functions/.test(prod),'Produção Spark não pode usar cloud-functions na jornada pública.');
