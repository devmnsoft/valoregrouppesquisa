...
public sealed class EnterpriseController(EnterpriseService service, IConfiguration configuration, IHostEnvironment hostEnvironment, ILogger<EnterpriseController> logger) : ControllerBase {
    ...
    public IActionResult ReleaseInfo() {
        ...
        return Ok(new {
            product = "Valora Insight™",
            version = configuration["Valora:Version"] ?? "8.0.0",
            releaseDate = configuration["Valora:ReleaseDate"] ?? "2026-08-07",
            environment,
            build = configuration["Valora:BuildId"] ?? "local",
            isHomologation = environment == "homologation"
        });
    }
    ...
    public async Task<IActionResult> CompanyStatus(Guid id, [FromBody] ChangeStatusRequest request, CancellationToken ct) { if (!IsValoraAdmin) return PermissionDenied(); await service.ChangeCompanyStatusAsync(id, request.Status, UserId, ct); return NoContent(); }
    ...
    [HttpPost("crm/leads")]
    public async Task<IActionResult> CreateLead([FromBody] CreateLeadRequest request, CancellationToken ct) { if (!IsValoraAdmin) return PermissionDenied(); var id = await service.CreateLeadAsync(request.Name, request.CompanyName, request.Email, request.Phone, request.IntendedPlan, request.Owner, request.NextActionAt, request.Notes, UserId, ct); return Created($"/api/v1/enterprise/crm/leads/{id}", new { id }); }

    [HttpGet("items/{kind}")]
    public async Task<IActionResult> Items(string kind, CancellationToken ct) { var scope = IsValoraAdmin ? null : OrganizationId; return Ok(await service.ItemsAsync(scope, kind, ct)); }

    [HttpPost("items")]
    public async Task<IActionResult> CreateItem([FromBody] UpsertEnterpriseItemRequest request, CancellationToken ct) { if (!IsValoraAdmin && IsValoraOnly(request.Kind)) return PermissionDenied(); var id = await service.SaveItemAsync(IsValoraAdmin ? null : OrganizationId, null, request, UserId, ct); return Created($"/api/v1/enterprise/items/{id}", new { id }); }

    [HttpPut("items/{id:guid}")]
    public async Task<IActionResult> UpdateItem(Guid id, [FromBody] UpsertEnterpriseItemRequest request, CancellationToken ct) { if (!IsValoraAdmin && IsValoraOnly(request.Kind)) return PermissionDenied(); await service.SaveItemAsync(IsValoraAdmin ? null : OrganizationId, id, request, UserId, ct); return NoContent(); }

    [HttpPost("api-keys")]
    public async Task<IActionResult> ApiKey([FromBody] CreateApiKeyRequest request, CancellationToken ct) { return StatusCode(410, new { code = "ENDPOINT_MOVED", message = "Use /api/v1/api-keys. A criação exige entitlement Enterprise.", correlationId = HttpContext.TraceIdentifier }); }

    [RequestSizeLimit(2_000_000), HttpPost("imports/{type}/preview")]
    public IActionResult Preview(string type, [FromBody] CsvPreviewRequest request) { if (OrganizationId is null && !IsValoraAdmin) return PermissionDenied(); return Ok(service.PreviewCsv(type, request.Content)); }
