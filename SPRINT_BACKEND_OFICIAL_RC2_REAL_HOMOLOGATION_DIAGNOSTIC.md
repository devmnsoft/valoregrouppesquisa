...
3. **`backend-v2`**: permanece referência histórica e fora do build oficial.
4. **Scripts SQL**: scripts oficiais estão em `database/postgresql` e no `scriptbd_completo.sql` raiz; arquivos de archive contêm histórico e não são build oficial.
...
20. **Plano objetivo da sprint**: executar validadores disponíveis, corrigir falhas reais detectadas, registrar limitações de ambiente, adicionar documentação/validador RC2 e preparar RC2 sem alterar legado ou `backend-v2`.
