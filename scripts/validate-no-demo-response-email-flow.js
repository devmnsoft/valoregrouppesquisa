...
const allowed=f=>/\.(test|spec)\./.test(f)||f.endsWith('.md')||f.includes('validate-legacy-email-real-response-flow.js')||f.includes('validate-no-demo-response-email-flow.js');const skip=new Set(['node_modules','.git','dist','reports','backups']);const hits=[];
