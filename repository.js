...
async function read(method,args){if(firebaseOnlyPublicOps()&&['validatePublicSurvey','loadPublicResult','getParticipantResultsByPassword'].includes(method)){const p=firebaseRepository();if(!p?.[method])throw new Error(`Firebase provider não possui método ${method}.`);return p[method](...args);}
...
async function write(method,args){if(firebaseOnlyPublicOps()&&['submitPublicSurveyResponse','sendResultEmail','requestNewResultLink'].includes(method)){const p=firebaseRepository();if(!p?.[method])throw new Error(`Firebase provider não possui método ${method}.`);return p[method](...args);}
