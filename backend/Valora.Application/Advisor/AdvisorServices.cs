...
public sealed class AdvisorGuardrailService(IAdvisorGuardrailRepository repository) {
    public async Task Validate(Guid o, Guid u, Guid? conversation, int requestedSourceCount, IReadOnlyList<AdvisorContextOptionDto> evidence, CancellationToken ct) {
        if (evidence.Count == 0) { await Block("evidence.required", "Uma resposta analítica foi bloqueada porque nenhuma fonte verificável foi selecionada.", "Dados insuficientes: selecione ao menos uma fonte verificável. O Advisor não conclui sem evidência."); return; }
        if (evidence.Count != requestedSourceCount) { await Block("tenant.source_rejected", "Uma ou mais fontes solicitadas não existem ou não pertencem à organização selecionada.", "Uma fonte selecionada não está disponível nesta organização. A análise foi bloqueada para proteger o isolamento dos dados."); }
        async Task Block(string rule, string reason, string userMessage) { await repository.Record(o, u, conversation, rule, reason, ct); throw new InvalidOperationException(userMessage); }
    }
}
public sealed class AdvisorResponseComposerService(IAdvisorModelProvider provider) {
    ...
    public async Task<AdvisorModelResult> Compose(string question, IReadOnlyList<AdvisorContextOptionDto> evidence, CancellationToken ct) { var result = await provider.Generate(new(Methodology, question, evidence), ct); if (result.ProviderUsed) return result; var sources = string.Join("; ", evidence.Select(x => $"{x.Title} ({x.SourceType})")); return result with { Content = $"Leitura orientadora baseada nas fontes selecionadas: {sources}. As evidências permitem organizar a investigação, mas não demonstram causalidade isoladamente. Próximo passo recomendado: validar a interpretação com os responsáveis e reunir evidências convergentes antes de decidir." }; }
}
