...
[ApiController,Authorize,Route("users")] public sealed class UsersController(IUserRepository repo,IPasswordHasher hasher,IAuditService audit,IEntitlementService entitlements):ApiBase{
 static readonly string[] AllowedRoles=ValoraPesquisa.Domain.Enums.Roles.All;
 ...
 [HttpPost] public async Task<IActionResult> Post(UserInput i){ var cur=Current(); if(!AllowedRoles.Contains(i.Role)) throw new ArgumentException("Perfil inválido."); var org=cur.IsAdminValora?i.OrganizationId:cur.OrganizationId; if(!cur.IsAdminValora && i.Role==Roles.AdminValora) return Forbid(); if(org.HasValue){var gate=await entitlements.CanCreateUserAsync(org.Value,i.Role is Roles.EmpresaAdmin or Roles.GestorPesquisa or Roles.GestorArea); if(!gate.Allowed){await audit.RecordAsync(org,cur.Id,gate.Code=="PLAN_LIMIT_REACHED"?"commercial_limit_reached":"commercial_module_blocked","user",null,Cid,new{gate.Code,gate.Limit,gate.Used}); return StatusCode(gate.Code=="PLAN_LIMIT_REACHED"?409:403,gate);}} var u=new User(Guid.NewGuid(),org,i.Name,i.Email,hasher.Hash(i.Password),i.Role,"active",i.Phone,null,DateTimeOffset.UtcNow,null,false); var saved=await repo.CreateAsync(u); await audit.RecordAsync(saved.OrganizationId,cur.Id,"user_created","user",saved.Id,Cid,new{saved.Email,saved.Role}); return Ok(saved);}
