...
## Consolidação oficial `backend` x `projeto .NET predecessor removido`

- A base oficial passa a ser exclusivamente `backend/Valora.sln`.
- `projeto .NET predecessor removido` permanece apenas como referência temporária e não deve receber novas features.
