...
public sealed class MethodologyRepository(IDbConnectionFactory connections) : IMethodologyRepository
{
    ...
    {
        const string sql = "SELECT id,code,name,pillar,definition,strategic_purpose StrategicPurpose,evolution_guidance EvolutionGuidance,diagnostic_questions DiagnosticQuestions,related_indicators RelatedIndicators,related_indices RelatedIndices,organizational_impacts OrganizationalImpacts,deprecated_terms DeprecatedTerms,maturity_level MaturityLevel,methodology_version MethodologyVersion,status,display_order DisplayOrder,version FROM valorapesquisa.methodology_concepts WHERE deleted_at IS NULL AND status='active' AND (@search IS NULL OR name ILIKE '%'||@search||'%' OR definition ILIKE '%'||@search||'%') AND (@pillar IS NULL OR pillar=@pillar) ORDER BY display_order,pillar,name";
    ...
    { using var c = connections.Create(); return await c.QuerySingleOrDefaultAsync<MethodologyConceptDto>(new CommandDefinition("SELECT id,code,name,pillar,definition,strategic_purpose StrategicPurpose,evolution_guidance EvolutionGuidance,diagnostic_questions DiagnosticQuestions,related_indicators RelatedIndicators,related_indices RelatedIndices,organizational_impacts OrganizationalImpacts,deprecated_terms DeprecatedTerms,maturity_level MaturityLevel,methodology_version MethodologyVersion,status,display_order DisplayOrder,version FROM valorapesquisa.methodology_concepts WHERE lower(code)=lower(@code) AND deleted_at IS NULL", new { code }, cancellationToken: ct)); }
