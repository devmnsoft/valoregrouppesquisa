...
for (const [name, cmd] of officialBuildScripts) {
  if (name.startsWith('projeto .NET predecessor removido')) continue;
  ok(!/projeto .NET predecessor removido\//.test(cmd), `script oficial ${name} aponta para projeto .NET predecessor removido`);
}
