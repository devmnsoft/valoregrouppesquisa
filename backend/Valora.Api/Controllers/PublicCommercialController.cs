...
public sealed class PublicCommercialController(PublicCommercialService service) : ControllerBase {
    ...
    public async Task<IActionResult> Contact([FromBody] CommercialRequestInput request, CancellationToken ct) { try { var id = await service.RequestContactAsync(request, ct); return Created($"/api/v1/public/contact-requests/{id}", new { id, status = "requested", message = "Solicitação registrada. O plano não foi alterado.", correlationId = CorrelationId }); } catch (ArgumentException ex) { return BadRequest(new { code = "VALIDATION_ERROR", message = ex.Message, correlationId = CorrelationId }); } }
}
