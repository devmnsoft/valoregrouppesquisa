...
public sealed class ProcessInstanceService(IProcessInstanceRepository repository) {
    public Task<ProcessDashboardDto> Dashboard(Guid o, CancellationToken ct) { ProcessScope.Ensure(o); return repository.Dashboard(o, ct); }
    public Task<IReadOnlyList<ProcessInstanceDto>> List(Guid o, CancellationToken ct) { ProcessScope.Ensure(o); return repository.List(o, ct); }
    public Task<ProcessInstanceDto?> Get(Guid o, Guid id, CancellationToken ct) { ProcessScope.Ensure(o); return repository.Get(o, id, ct); }
