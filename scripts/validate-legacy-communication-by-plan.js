...
const missing=["buildWhatsappMessageByContext","buildResultEmailByPlan","participant_result","admin_result"].filter(x=>!corpus.includes(x));
if(missing.length){console.error('Missing legacy validation markers for communication-by-plan:',missing.join(', '));process.exit(1);}
...
console.log('OK legacy communication-by-plan');
