...
async function resolveFeaturedHomeSurveyPublic(){
  ...
  try{
    ...
    if(remote?.ok&&remote?.url&&!/survey_demo|empresa-exemplo|tokenHash=/i.test(remote.url)){
      const cacheKey=remote.survey?.id||'';
      if(cacheKey)publicSurveyCache.set(cacheKey,{survey:remote.survey,form:remote.form,company:remote.company,token:remote.token});
      if(session.store){
        if(remote.survey&&!session.store.surveys?.some(x=>x.id===remote.survey.id))session.store.surveys.push(remote.survey);
        if(remote.form&&!session.store.forms?.some(x=>x.id===remote.form.id))session.store.forms.push(remote.form);
        if(remote.company&&!session.store.companies?.some(x=>x.id===remote.company.id))session.store.companies.push(remote.company);
      }
      window.ValoraRuntimeDiagnostics=window.ValoraRuntimeDiagnostics||{};
      window.ValoraRuntimeDiagnostics.lastFeaturedHomeSurvey={attemptedSources:attempted,reason:'callable_candidate_found',surveyId:remote.survey?.id||'',hasPublicToken:!!remote.token,org:remote.org||remote.company?.slug||'',source:remote.source||'cloud-function'};
      return {survey:remote.survey,company:remote.company,form:remote.form,url:remote.url,token:remote.token,org:remote.org,source:remote.source||'cloud-function'};
    }
