...
-- Converge instalações que receberam a massa demo anterior sem criar outra organização.
UPDATE valorapesquisa.organizations
SET name='Empresa Demonstração Valora',slug='empresa-demonstracao-valora',status='active',deleted_at=NULL,updated_at=now()
WHERE slug='organizacao-demo-valora'
  AND NOT EXISTS (SELECT 1 FROM valorapesquisa.organizations current_demo WHERE current_demo.slug='empresa-demonstracao-valora');

INSERT INTO valorapesquisa.organizations(name,slug,status,created_at,updated_at,deleted_at)
VALUES ('Empresa Demonstração Valora','empresa-demonstracao-valora','active',now(),now(),NULL)
...
SELECT o.id,'admin.demo@valora.local','Administrador Demo [DEMO]',
       public.crypt('Valora@123456',public.gen_salt('bf',12)),'active',false,now(),now(),NULL
FROM valorapesquisa.organizations o WHERE o.slug='empresa-demonstracao-valora'
...
WHERE o.slug='empresa-demonstracao-valora' AND lower(u.email)='admin.demo@valora.local'
...
WHERE o.slug='empresa-demonstracao-valora'
...
SELECT o.id,'Empresa Demonstração Valora','Valora Demonstração','00000000000000','active',now(),now(),NULL
FROM valorapesquisa.organizations o WHERE o.slug='empresa-demonstracao-valora'
...
FROM valorapesquisa.organizations WHERE slug='empresa-demonstracao-valora'
ON CONFLICT(organization_id) DO UPDATE
SET settings=COALESCE(organization_settings.settings,'{}'::jsonb) || EXCLUDED.settings,updated_at=now();

COMMIT;
\echo 'Seed demo aplicado. Login: admin.demo@valora.local | senha local: Valora@123456'
