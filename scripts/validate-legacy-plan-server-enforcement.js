...
const missing=["VALORA_SERVER_PLAN_CATALOG","enforceServerPlanLimit","monthlyResponses","countMonthlyResponses","plan_limit_reached"].filter(x=>!corpus.includes(x));
if(missing.length){console.error('Missing legacy validation markers for plan-server-enforcement:',missing.join(', '));process.exit(1);}
...
console.log('OK legacy plan-server-enforcement');
