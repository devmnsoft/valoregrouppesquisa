(function(){
'use strict';
const EMAIL_TRANSPORTS=new Set(['disabled','local-outbox','firebase-functions','external-api','auto']);
...
function normalizeProvider(value,fallback){return ['api','external-api','firebase-functions','cloud-functions','local','auto'].includes(value)?(value==='cloud-functions'?'firebase-functions':value):fallback;}
function getRuntimeCapabilities(){
  ...
  const functions=functionsEnabled&&(transport==='firebase-functions'||transport==='auto');
  const externalApi=(transport==='external-api'||transport==='auto')&&gatewayEnabled;
  ...
  const usesCloudFunctions=functionsEnabled&&([validationProvider,submissionProvider,resultProvider].includes('firebase-functions')||[validationProvider,submissionProvider,resultProvider].includes('auto'));
  const usesGateway=[validationProvider,submissionProvider,resultProvider].includes('external-api')||[validationProvider,submissionProvider,resultProvider].includes('auto');
  const usesApi=[validationProvider,submissionProvider,resultProvider].includes('api');
  const canValidateSurvey=validationProvider==='auto'||validationProvider==='local'||(validationProvider==='api'&&Boolean(apiBaseUrl))||(validationProvider==='external-api'&&gatewayEnabled)||(validationProvider==='firebase-functions'&&usesCloudFunctions);
  const canSubmitResponse=submissionProvider==='auto'||submissionProvider==='local'||(submissionProvider==='api'&&Boolean(apiBaseUrl))||(submissionProvider==='external-api'&&gatewayEnabled)||(submissionProvider==='firebase-functions'&&usesCloudFunctions);
  const canLoadResult=resultProvider==='auto'||resultProvider==='local'||(resultProvider==='api'&&Boolean(apiBaseUrl))||(resultProvider==='external-api'&&gatewayEnabled)||(resultProvider==='firebase-functions'&&usesCloudFunctions);
  ...
  return Object.freeze({
    ...
    communicationGateway:Object.freeze({enabled:gatewayEnabled,baseUrl:gatewayBase,mode:gateway.mode||'disabled',canSendResult:(transport==='external-api'||transport==='auto')&&gatewayEnabled&&gateway.sendResultOnSurveyCompleted===true,allowManualResend:gateway.allowManualResend===true,timeoutMs:Number(gateway.timeoutMs||20000)}),
