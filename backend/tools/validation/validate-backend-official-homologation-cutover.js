...
for (const doc of docs) {
  ...
  if (/evoluir\s+`?projeto .NET predecessor removido`?/i.test(txt)) fail(`${doc} appears to instruct evolving projeto .NET predecessor removido`);
}
const officialBuildRefs = JSON.stringify(pkg.scripts).match(/projeto .NET predecessor removido:(build|official|prod)/i);
if (officialBuildRefs) fail('projeto .NET predecessor removido referenced as official build path'); else ok('projeto .NET predecessor removido is not official build path');
