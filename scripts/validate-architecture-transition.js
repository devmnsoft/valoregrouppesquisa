...
const checks=[
 ...
 ['database scripts existem',['001_create_schemas.sql','002_core_tables.sql','003_plan_tables.sql','004_survey_tables.sql','005_response_tables.sql','006_certificate_tables.sql','007_communication_tables.sql','008_audit_tables.sql','009_seed_official_plans.sql'].every(f=>has(`backend/database/postgresql/${f}`))],
 ['API health existe',/\/health/.test(read('backend/Valora.Api/Controllers/HealthController.cs'))],
 ['schema tem tabelas principais',/valora\.organizations/.test(read('backend/database/postgresql/002_core_tables.sql'))&&/billing\.plans/.test(read('backend/database/postgresql/003_plan_tables.sql'))&&/valora\.responses/.test(read('backend/database/postgresql/005_response_tables.sql'))],
 ['seed dos planos existe',['free','essential','professional','corporate','enterprise'].every(p=>read('backend/database/postgresql/009_seed_official_plans.sql').includes(`'${p}'`))],
