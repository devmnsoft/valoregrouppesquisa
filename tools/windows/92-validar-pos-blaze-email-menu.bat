...
echo [4/35] Cache busting...
call npm run config:cache-busting
if errorlevel 1 goto erro

echo [5/35] Functions production deployment...
call npm run functions:production-deployment
if errorlevel 1 goto erro

echo [6/35] Readiness SMTP Functions...
call npm run email:blaze-functions-readiness
if errorlevel 1 goto erro

echo [7/35] Provider auto submissao...
call npm run home:submission-auto-provider
if errorlevel 1 goto erro

echo [8/35] Pesquisa gratis sem expirar...
call npm run home:free-survey-never-expires
if errorlevel 1 goto erro

echo [9/35] Email apos submit...
call npm run home:email-after-submit
if errorlevel 1 goto erro

echo [10/35] Sem resposta demonstrativa...
call npm run email:no-demo-response
if errorlevel 1 goto erro

echo [11/35] Menu mobile lista completa...
call npm run admin:mobile-menu-full-list
if errorlevel 1 goto erro

echo [12/35] Menu mobile runtime...
call npm run admin:mobile-menu-runtime
if errorlevel 1 goto erro

echo [13/35] Diagnostico ambiente legado...
call npm run legacy:environment-diagnostics
if errorlevel 1 goto erro

echo [14/35] Paridade Valora.Web...
...
if not exist SPRINT_58_POST_BLAZE_PRODUCTION_AUDIT.md goto erro

echo POS-BLAZE + EMAIL + MENU MOBILE VALIDADO.
pause
exit /b 0

:erro
echo FALHA NA VALIDACAO POS-BLAZE + EMAIL + MENU MOBILE.
pause
exit /b 1
