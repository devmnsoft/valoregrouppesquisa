...
`dotnet build projeto .NET predecessor removido/solution predecessora removida` foi tentado em 2026-07-01, mas o container retornou `dotnet: command not found`. O build real deve ser executado em ambiente com .NET SDK 8. Scripts e README documentam o caminho de execução.

## 4. Testes executados ou motivo de não execução
`dotnet test projeto .NET predecessor removido/solution predecessora removida` não pôde rodar pelo mesmo motivo ambiental: ausência do SDK. O validador automatizado Node foi executado com sucesso via `npm run projeto .NET predecessor removido:validate`.
...
## 7. DTOs organizados
Os DTOs foram organizados em subpastas sob `Application/DTOs` por domínio funcional: organizações, usuários, formulários, pesquisas, público, respostas, resultados e auditoria, preservando o namespace `Valora.Application.DTOs`.
...
Foi criado `projeto .NET predecessor removido/docker-compose.yml` com PostgreSQL, `Valora.Api` e `Valora.Web`, além de Dockerfiles para API e Web. O compose usa banco `valorapesquisa` e aplica o SQL versionado de inicialização.
...
## 16. Validador criado
Foi criado `projeto .NET predecessor removido/tools/validate-projeto .NET predecessor removido-foundation.js` e adicionado o script raiz `projeto .NET predecessor removido:validate`. A execução atual passou com sucesso.
...
Instalar .NET SDK 8 no CI/agente, executar `dotnet build projeto .NET predecessor removido/solution predecessora removida` e `dotnet test projeto .NET predecessor removido/solution predecessora removida`, ativar PostgreSQL descartável para testes integrados e então iniciar a Sprint 04 — planos, módulos, limites comerciais, bloqueio por assinatura, dashboard real e permissões avançadas.
