...
public sealed class MigrationDryRunService(IEnumerable<ILegacySourceReader> readers, IMigrationRecordRepository records, IMigrationConflictRepository conflicts, IMigrationBatchRepository batches, IAuditRepository audit) : IMigrationDryRunService, ILegacyImportService
{
    ...
    {
        await audit.AddAsync(new AuditEntry(null,null,"migration.dry_run.started","migration_batch",request.BatchId.ToString(),"Dry-run iniciado","{}"));
        var started=DateTime.UtcNow; var recs=new List<MigrationRecordDto>(); var cons=new List<MigrationConflictDto>(); var unmapped=new HashSet<string>(); var sensitive=new HashSet<string>(); var total=0; var invalid=0;
        try{
        foreach(var src in request.Sources){ct.ThrowIfCancellationRequested(); var reader=readers.FirstOrDefault(r=>r.CanRead(src.SourceType))??throw new InvalidOperationException("Fonte de importação não suportada."); var data=await reader.ReadAsync(src,ct); foreach(var d in data.Documents){ct.ThrowIfCancellationRequested(); total++; foreach(var u in d.UnmappedFields) unmapped.Add($"{d.Collection}.{u}"); foreach(var s in d.SensitiveFields) sensitive.Add($"{d.Collection}.{s}"); var status=d.TargetEntity=="manual_review"?"invalid":"planned"; if(status=="invalid") invalid++; var dto=new MigrationRecordDto(Guid.NewGuid(),request.BatchId,null,d.Collection,d.LegacyId,d.TargetEntity,null,"insert",status,d.MaskedJson,d.NormalizedMaskedJson,status=="invalid"?"UNMAPPED_COLLECTION":null,status=="invalid"?"Coleção sem destino oficial claro.":null); recs.Add(dto); await records.AddAsync(dto,ct); if(status=="invalid"){var c=new MigrationConflictDto(Guid.NewGuid(),request.BatchId,d.Collection,d.LegacyId,d.TargetEntity,null,"unmapped_collection","blocking",d.MaskedJson,"{}",null,null,null); cons.Add(c); await conflicts.AddAsync(c,ct); await audit.AddAsync(new AuditEntry(null,null,"migration.conflict.registered","migration_conflict",c.Id.ToString(),"Conflito registrado sem payload sensível","{}"));}}}
        ...
        await batches.UpdateStatusAsync(request.BatchId,statusFinal,JsonSerializer.Serialize(new { total, batchSize = Environment.GetEnvironmentVariable("VALORA_MIGRATION_BATCH_SIZE") ?? "500", elapsedMs = (long)(DateTime.UtcNow-started).TotalMilliseconds }),ct);
        await audit.AddAsync(new AuditEntry(null,null,"migration.dry_run.completed","migration_batch",request.BatchId.ToString(),"Dry-run concluído","{}"));
        return new MigrationValidationReportDto(request.BatchId, statusFinal, sum, recs, cons);
        } catch { await audit.AddAsync(new AuditEntry(null,null,"migration.dry_run.failed","migration_batch",request.BatchId.ToString(),"Dry-run falhou","{}")); throw; }
    }
}
public sealed class MigrationApplyService(IMigrationBatchRepository batches, IMigrationConflictRepository conflicts, IMigrationMappingRepository mappings, IMigrationRecordRepository records, IMigrationRollbackRepository rollbacks, IAuditRepository audit) : IMigrationApplyService
{
    public async Task<MigrationReconciliationReportDto> ApplyAsync(MigrationApplyRequest request,CancellationToken ct=default)
    {
        await audit.AddAsync(new AuditEntry(null,null,"migration.apply.started","migration_batch",request.BatchId.ToString(),"Apply iniciado","{}"));
        try
        {
            if(!request.ConfirmApply) throw new InvalidOperationException("Importação real exige confirmApply=true.");
            if(request.RequestedByRole!="admin_valora") throw new UnauthorizedAccessException("Apenas admin_valora pode aplicar importação nesta sprint.");
            var batch=await batches.GetAsync(request.BatchId,ct)??throw new InvalidOperationException("Batch não encontrado.");
            if(!batch.Status.Contains("dry_run") && batch.Status!="blocked") throw new InvalidOperationException("Apply exige dry-run anterior.");
            if(await conflicts.HasBlockingAsync(request.BatchId,ct)) throw new InvalidOperationException("Conflito bloqueante impede apply.");
            var all=(await records.ListByBatchAsync(request.BatchId,ct)).Where(x=>x.Status!="invalid").ToList();
            var batchSize=Math.Max(1,int.TryParse(Environment.GetEnvironmentVariable("VALORA_MIGRATION_BATCH_SIZE"),out var b)?b:500);
            foreach(var chunk in all.Chunk(batchSize))
            {
                foreach(var r in chunk)
                {
                    ...
                    await rollbacks.AddAsync(new MigrationRollbackItemDto(Guid.NewGuid(),request.BatchId,r.TargetEntity,target,"insert",null,r.NormalizedMaskedJson,"planned",null),ct);
                    await mappings.AddAsync(new MigrationMappingDto(Guid.NewGuid(),request.BatchId,r.LegacyCollection,r.LegacyId??r.Id.ToString(),r.TargetEntity,target,$"{r.LegacyCollection}:{r.LegacyId}"),ct);
                }
            }
            await batches.UpdateStatusAsync(request.BatchId,"applied",JsonSerializer.Serialize(new { sensitive="masked", batchSize }),ct);
            await audit.AddAsync(new AuditEntry(null,null,"migration.apply.completed","migration_batch",request.BatchId.ToString(),"Apply concluído","{}"));
            return new(request.BatchId,"ready_with_warnings",new Dictionary<string,int>(),new Dictionary<string,int>{{"records",all.Count}},Array.Empty<string>());
        }
        catch
        {
            await audit.AddAsync(new AuditEntry(null,null,"migration.apply.failed","migration_batch",request.BatchId.ToString(),"Apply falhou","{}"));
            throw;
        }
    }
}
public sealed class MigrationReconciliationService(IMigrationRecordRepository records, IMigrationConflictRepository conflicts, IAuditRepository audit) : IMigrationReconciliationService { public async Task<MigrationReconciliationReportDto> ReconcileAsync(Guid batchId,CancellationToken ct=default){var r=await records.ListByBatchAsync(batchId,ct); var c=await conflicts.ListByBatchAsync(batchId,ct); await audit.AddAsync(new AuditEntry(null,null,"migration.reconciliation.executed","migration_batch",batchId.ToString(),"Conciliação executada","{}")); return new(batchId,c.Any(x=>x.Severity=="blocking")?"blocked":"ready_with_warnings",r.GroupBy(x=>x.TargetEntity).ToDictionary(g=>g.Key,g=>g.Count()),r.Where(x=>x.Status!="invalid").GroupBy(x=>x.TargetEntity).ToDictionary(g=>g.Key,g=>g.Count()),c.Select(x=>$"{x.TargetEntity}:{x.ConflictType}").ToArray());}}
public sealed class MigrationRollbackService(IMigrationRollbackRepository repo, IAuditRepository audit) : IMigrationRollbackService { public Task<IReadOnlyList<MigrationRollbackItemDto>> GetReportAsync(Guid batchId,CancellationToken ct=default)=>repo.ListByBatchAsync(batchId,ct); public async Task<MigrationReconciliationReportDto> RollbackAsync(MigrationRollbackRequest request,CancellationToken ct=default){ await audit.AddAsync(new AuditEntry(null,null,"migration.rollback.started","migration_batch",request.BatchId.ToString(),"Rollback iniciado","{}")); try{ if(!request.ConfirmRollback) throw new InvalidOperationException("Rollback exige confirmRollback=true."); if(request.RequestedByRole!="admin_valora") throw new UnauthorizedAccessException("Apenas admin_valora pode executar rollback."); var items=await repo.ListByBatchAsync(request.BatchId,ct); foreach(var i in items) await repo.MarkRolledBackAsync(i.Id,ct); await audit.AddAsync(new AuditEntry(null,null,"migration.rollback.completed","migration_batch",request.BatchId.ToString(),"Rollback concluído","{}")); return new(request.BatchId,"rolled_back",new Dictionary<string,int>(),new Dictionary<string,int>{{"rollback_items",items.Count}},Array.Empty<string>()); } catch { await audit.AddAsync(new AuditEntry(null,null,"migration.rollback.failed","migration_batch",request.BatchId.ToString(),"Rollback falhou","{}")); throw; } }}
public sealed class CutoverReadinessService(IMigrationConflictRepository conflicts, IAuditRepository audit) : ICutoverReadinessService { public async Task<CutoverReadinessDto> GetAsync(Guid batchId,CancellationToken ct=default){var c=await conflicts.ListByBatchAsync(batchId,ct); var blockers=c.Where(x=>x.Severity=="blocking").Select(x=>$"{x.TargetEntity}:{x.ConflictType}").ToArray(); await audit.AddAsync(new AuditEntry(null,null,"migration.cutover_readiness.generated","migration_batch",batchId.ToString(),"Readiness de cutover gerado","{}")); return new(batchId,blockers.Length>0?"blocked":"ready_with_warnings",new[]{"Dry-run executado","Conciliação revisada","Rollback planejado","Auditoria ativa"},blockers,c.Where(x=>x.Severity!="blocking").Select(x=>x.ConflictType).ToArray(),"Executar janela manual, congelar legado, aplicar batch e validar amostras.","Executar rollback por batch e reativar legado preservado."); } }
