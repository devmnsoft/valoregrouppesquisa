...
function emptyStore(seedStore,normalizeState){const state=seedStore();Object.assign(state,{session:null,users:[],companies:[],forms:[],surveys:[],responses:[],invitations:[],invoices:[],actionPlans:[],integrations:[],apiKeys:[],webhooks:[],integrationLogs:[],logs:[]});normalizeState(state);return state;}
...
async function loadStoreData(){
  ...
  const [organizations,users,plans,modules,forms,surveys,responses,invitations,invoices,actionPlans,notifications,settings,logs,integrations,apiKeys,webhooks,integrationLogs]=await Promise.all([
  ...
  return {session:{userId:p.id||p.uid,createdAt:new Date().toISOString()},companies:organizations.map(normalizeCompany),organizations,users,plans,modules,forms,surveys,responses,invitations,invoices,actionPlans,notifications,settings,logs,integrations,apiKeys,webhooks,integrationLogs};
}
...
window.ValoraFirebaseRepository={
  ...
  async saveChanges({state}={}){session.store=state||session.store;if(!session.profile||!session.loaded)return session.store;try{await Promise.all(['companies','users','plans','modules','forms','surveys','invitations','invoices','actionPlans','notifications','integrations','apiKeys','webhooks','integrationLogs'].map(syncCollectionFromState));await syncSettings();}catch(err){console.warn('[Valora Pulse] Falha ao salvar no Firestore.',err);throw mapCollectionError(err);}return session.store;},
