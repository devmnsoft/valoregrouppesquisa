...
public sealed class ActionCenterController(ActionPlanService plans, ActionItemService items, ActionProgressService progress) : Controller {
    ...
    [HttpGet("Items")] public async Task<IActionResult> Items(CancellationToken c) => View(await items.List(O, null, c)); [HttpGet("Items/Details/{id:guid}")] public async Task<IActionResult> ItemDetails(Guid id, CancellationToken c) { var i = await items.Get(O, id, c); return i is null ? NotFound() : View(i); }
    [ValidateAntiForgeryToken, HttpPost("Items/{id:guid}/Progress")] public async Task<IActionResult> Update(Guid id, int progressPercent, string? note, CancellationToken c) { await progress.Update(O, U, id, progressPercent, note, c); return RedirectToAction(nameof(ItemDetails), new { id }); }
    [ValidateAntiForgeryToken, HttpPost("Items/{id:guid}/Block")] public async Task<IActionResult> Block(Guid id, string reason, CancellationToken c) { await progress.Block(O, U, id, reason, c); return RedirectToAction(nameof(ItemDetails), new { id }); }
    [ValidateAntiForgeryToken, HttpPost("Items/{id:guid}/Complete")] public async Task<IActionResult> Complete(Guid id, string result, string evidence, CancellationToken c) { await progress.Complete(O, U, id, result, evidence, c); return RedirectToAction(nameof(ItemDetails), new { id }); }
}
