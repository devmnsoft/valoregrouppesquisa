...
public sealed class MigrationImportTests
{
    ...
    public void Normalizes_Email_Document_Status_Role_Module()
    {
        ...
        Assert.Equal("manager", normalizer.NormalizeRole("gestor"));
        Assert.Equal("pesquisa-clima", normalizer.NormalizeModule("Pesquisa Clima"));
    }
    ...
    public async Task Firestore_Reader_Masks_Sensitive_Data()
    {
        ...
        var result = await reader.ReadAsync(new MigrationUploadRequest(
            ...
            "f.json",
            "application/json",
            "{\"users\":{\"u1\":{\"email\":\"A@B.COM\",\"password\":\"plain\",\"token\":\"abc\"}}}"));

        Assert.Single(result.Documents);
        Assert.Contains("***MASKED***", result.Documents[0].MaskedJson);
        Assert.DoesNotContain("plain", result.Documents[0].MaskedJson);
    }
    ...
    public async Task LocalStorage_Reader_Accepts_Array_Structure()
    {
        ...
        var result = await reader.ReadAsync(new MigrationUploadRequest(
            ...
            "application/json",
            "{\"forms\":[{\"id\":\"f1\",\"title\":\"Clima\"}]}"));

        Assert.Equal("forms", result.Documents[0].Collection);
        Assert.Equal("forms", result.Documents[0].TargetEntity);
    }
    ...
    public void MaskSensitiveJson_Masks_Password_Token_And_SmtpSecret()
    {
        ...
        Assert.Contains("\"password\":\"***MASKED***\"", masked);
        Assert.Contains("\"token\":\"***MASKED***\"", masked);
        Assert.Contains("\"smtpSecret\":\"***MASKED***\"", masked);
        Assert.Contains("\"name\":\"ok\"", masked);
        Assert.DoesNotContain("plain", masked);
        Assert.DoesNotContain("abc", masked);
        Assert.DoesNotContain("smtp-value", masked);
    }
    ...
    public void MaskSensitiveJson_Preserves_Arrays_Numbers_And_Booleans()
    {
        ...
        Assert.Equal("***MASKED***", root.GetProperty("items")[1].GetProperty("token").GetString());
        Assert.DoesNotContain("hidden", masked);
    }
    ...
    public void MaskSensitiveJson_Does_Not_Leak_Sensitive_Values()
    {
        ...
        Assert.DoesNotContain("refresh-secret", masked);
        Assert.DoesNotContain("Server=prod", masked);
        Assert.DoesNotContain("hash-value", masked);
        Assert.Equal(4, masked.Split("***MASKED***").Length - 1);
    }
    ...
    public void Apply_And_Rollback_Requests_Expose_Confirmations()
    {
        ...
        Assert.False(apply.ConfirmApply);
        Assert.False(rollback.ConfirmRollback);
    }
    ...
    public void Dtos_Do_Not_Expose_Raw_Sensitive_Payload_Names()
    {
        ...
        Assert.DoesNotContain("TokenHash", props);
    }
