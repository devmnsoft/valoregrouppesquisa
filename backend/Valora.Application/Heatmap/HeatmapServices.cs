...
public sealed class GenerateHeatmapSnapshotUseCase(IHeatmapService service) { public Task<HeatmapSnapshotDto> ExecuteAsync(Guid org, Guid? user, GenerateHeatmapRequest request, CancellationToken ct) => service.GenerateAsync(org, user, request, ct); }
public sealed class GetHeatmapOverviewUseCase(IHeatmapService service) { public Task<IReadOnlyList<HeatmapSnapshotDto>> ExecuteAsync(Guid org, CancellationToken ct) => service.OverviewAsync(org, ct); }
