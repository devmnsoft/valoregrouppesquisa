...
3. **Onde ainda existe demo-valora-insight?** Apenas em seed/teste local: `backend/database/postgresql/012_seed_demo_valora_insight.sql`.
...
22. **Onde ainda existe valora., billing., communication., audit. ou migration.?** Não encontrado como schema SQL canônico em `backend`, `backend/database/postgresql`, `migration` e `scripts`; menções textuais aparecem em nomes de scripts/documentação ou identificadores de frontend.
...
- `MigrationRunner` usa somente `valorapesquisa.schema_migrations`, lê apenas `.sql` diretamente em `backend/database/postgresql`, executa por arquivo em transação e não repete scripts aplicados.
