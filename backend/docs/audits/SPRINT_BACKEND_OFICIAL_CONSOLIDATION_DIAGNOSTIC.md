...
## 4. Itens do `projeto .NET predecessor removido` que não devem ser portados

Não portar a solution `solution predecessora removida`, nomes `ValoraPesquisa.*`, Dockerfiles paralelos como base oficial, scripts que apontam para `projeto .NET predecessor removido`, views duplicadas em outro frontend, seeds com tokens públicos em texto ou qualquer padrão que concorra com `backend/Valora.sln`.
...
Os scripts `projeto .NET predecessor removido:validate` e `projeto .NET predecessor removido:saas-validate` continuam apenas como referência histórica. O novo validador oficial é `backend:official-validate`, apontando para `tools/validate-backend-official-migration.js` e para `backend/Valora.sln`.

## 12. Riscos da consolidação

Riscos: rotas duplicadas, quebra de Web MVC, exposição acidental de hashes/tokens, divergência entre scripts SQL, controllers com regra pesada, uso indevido de `projeto .NET predecessor removido` em automações oficiais, seeds inseguros e endpoints administrativos com fallback não documentado.

## 13. Plano objetivo da sprint

1. Declarar `backend/Valora.sln` como fonte oficial.
2. Documentar mapa `projeto .NET predecessor removido` → `backend`.
