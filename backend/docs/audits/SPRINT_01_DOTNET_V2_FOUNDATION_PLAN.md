...
## 1. Decisão
Criar uma nova base paralela em `projeto .NET predecessor removido/solution predecessora removida`.
...
`Domain`, `Application`, `Infrastructure`, `Api`, `Web`, `Tests` e scripts PostgreSQL versionados em `projeto .NET predecessor removido/backend/database/postgresql`.
