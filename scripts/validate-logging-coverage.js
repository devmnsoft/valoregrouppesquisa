const fs=require('fs');const checks={
...
let fail=[];for(const [p,t] of Object.entries(checks)){const s=fs.readFileSync(p,'utf8');if(!s.includes(t))fail.push(`${p} sem ${t}`);if(!s.includes('LogInformation')&&!s.includes('LogWarning')&&!s.includes('LogError'))fail.push(`${p} sem logs`)} if(fail.length){console.error(fail.join('\n'));process.exit(1)} console.log('Cobertura de logging validada.');
