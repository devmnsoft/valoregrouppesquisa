...
public sealed class FormalDeliverablesTests {
    private static DiagnosisDocumentSnapshot Snapshot(bool completed = true) => new(
        ...
        ["Instituir revisão trimestral"], [new("Alta", "Criar fórum de indicadores", "Diretoria", DateOnly.FromDateTime(DateTime.UtcNow.AddDays(30)))],
    ...
    public void Xlsx_IsValidOpenXmlZipWithExpectedWorksheets() {
        ...
        Assert.Equal(10, zip.Entries.Count(x => x.FullName.StartsWith("xl/worksheets/sheet", StringComparison.Ordinal)));
        Assert.Contains(zip.Entries, x => x.FullName == "xl/workbook.xml");
        Assert.Contains(zip.Entries, x => x.FullName == "[Content_Types].xml");
    }
    ...
    public void Json_IsNonEmptyAndDoesNotExposeRespondentPersonalData() {
        ...
        Assert.False(json.RootElement.TryGetProperty("participantEmail", out _));
