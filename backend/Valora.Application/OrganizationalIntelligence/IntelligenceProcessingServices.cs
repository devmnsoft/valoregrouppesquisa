...
public sealed class IntelligenceStageLogger(IIntelligenceProcessingJobRepository repository) : IIntelligenceStageLogger { public Task LogAsync(Guid o, Guid j, Guid r, ProcessingStageResult s, string status, DateTime from, DateTime to, CancellationToken ct) => repository.LogStageAsync(o, j, r, s, status, from, to, ct); }
