...
public sealed class CertificateRepository(IDbConnectionFactory f, ILogger<CertificateRepository> logger):ICertificateRepository
{
 ...
 public async Task CreateMetadataAsync(Guid responseId,string validationCode,string status){try{using var c=f.Create(); await c.ExecuteAsync("INSERT INTO valorapesquisa.certificates(response_id,certificate_code,issuer_name,status) VALUES (@responseId,@validationCode,'Valora Insight™',@status) ON CONFLICT (certificate_code) DO UPDATE SET updated_at=now()",new{responseId,validationCode,status});}catch(Exception ex){logger.LogError(ex,"Erro ao criar metadados de certificado. ResponseId={ResponseId} Status={Status}",responseId,status);throw;}}
