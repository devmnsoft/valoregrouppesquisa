...
3. Perfis já existentes no projeto .NET predecessor removido antes da sprint: `admin_valora`, `empresa_admin`, `gestor_pesquisa`, `participante`.
...
5. Faltavam no projeto .NET predecessor removido tabelas, services e bloqueios formais para todos os módulos SaaS; havia apenas fluxo vertical mínimo.
...
* `dotnet build projeto .NET predecessor removido/solution predecessora removida`: não executado com sucesso porque o SDK `dotnet` não está instalado no ambiente (`dotnet: command not found`).
* `dotnet test projeto .NET predecessor removido/solution predecessora removida`: não executado com sucesso porque o SDK `dotnet` não está instalado no ambiente (`dotnet: command not found`).
* `npm run projeto .NET predecessor removido:validate`: executado com sucesso antes das mudanças; validador foundation OK.
