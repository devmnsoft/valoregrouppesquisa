...
function normalizeCallableError(err){return {code:String(err?.details?.code||err?.code||'').replace(/^functions\//,''),message:err?.message||'Erro na operação.',details:err?.details||{},customData:err?.customData||{},name:err?.name||'FirebaseError',rawCode:err?.code||''};}
function callFunction(name,payload){const s=ensureFirebase();if(!s.functions)throw Object.assign(new Error('Cloud Functions não configurado.'),{code:'functions-unavailable'});return s.functions.httpsCallable(name)(payload).then(r=>r.data).catch(err=>{const n=normalizeCallableError(err);const e=Object.assign(new Error(n.message),n,{originalError:err});throw e;});}
...
function correlationId(){return `vi_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,8)}`;}
function recordFirestoreError(collectionName,err,meta={}){const dbg=firestoreDebug();const p=session.profile||{};const item={collection:collectionName,operation:meta.operation||'read',role:p.role||'unknown',companyScopePresent:!!p.companyId,departmentScopePresent:!!p.department,errorCode:sanitizedErrorCode(err),retryable:['unavailable','deadline-exceeded','network'].includes(sanitizedErrorCode(err)),skipped:!!meta.skipped,timestamp:new Date().toISOString(),correlationId:correlationId()};dbg.errors.push(item);dbg.lastError=item;if(dbg.errors.length>80)dbg.errors=dbg.errors.slice(-80);console.error(`[Valora Insight] Erro ao carregar ${collectionName}`,item);}
...
function mapCollectionError(err){
  const code=err?.code||'';
  if(code==='permission-denied')return Object.assign(new Error('Não foi possível validar as permissões do seu acesso. Saia, entre novamente e tente outra vez.'),{code,category:'authorization',expected:true,retryable:false});
...
async function refreshAuthDebug(user=session.authUser){if(!user)return {};const tokenResult=await user.getIdTokenResult(false);const claims=tokenResult.claims||{};session.authDebug={uid:user.uid,email:user.email,claims,refreshedAt:new Date().toISOString(),tokenRefreshed:true,hasRoleClaim:!!claims.role};firestoreDebug().auth=session.authDebug;if(session.store)session.store.authDebug=session.authDebug;if(!claims.role)console.warn('[Valora Insight] Usuário autenticado, mas sem custom claim role. Faça logout/login ou reaplique claims.');return claims;}
...
async function loadStoreData(){
  ...
  const roles=(...values)=>values.includes(p.role);
  const tenant=()=>!!p.companyId&&!isGlobalRole(p);
  const manifest={
    organizations:{critical:tenant(),canLoad:()=>p.role!=='convidado_externo',load:loadOrganizations,fallback:[]},
    companiesRaw:{critical:false,canLoad:()=>p.role!=='convidado_externo',load:loadCompaniesRaw,fallback:[]},
    users:{critical:false,canLoad:()=>roles('admin_valora','consultor_valora','empresa_admin'),load:loadUsers,fallback:[]},
    plans:{critical:false,canLoad:()=>p.role!=='convidado_externo',load:()=>queryCollection('plans'),fallback:[]},
    modules:{critical:false,canLoad:()=>p.role!=='convidado_externo',load:()=>queryCollection('modules'),fallback:[]},
    forms:{critical:false,canLoad:()=>roles('admin_valora','consultor_valora','empresa_admin','gestor_pesquisa','analista_resultados','gestor_area'),load:loadForms,fallback:[]},
    surveys:{critical:false,canLoad:()=>roles('admin_valora','consultor_valora','empresa_admin','gestor_pesquisa','analista_resultados','gestor_area'),load:loadSurveys,fallback:[]},
    responses:{critical:false,canLoad:()=>p.role!=='convidado_externo',load:loadResponses,fallback:[]},
    invitations:{critical:false,canLoad:()=>roles('admin_valora','consultor_valora','empresa_admin','gestor_pesquisa'),load:()=>scopedRows('invitations'),fallback:[]},
    invoices:{critical:false,canLoad:()=>roles('admin_valora','empresa_admin'),load:()=>scopedRows('invoices'),fallback:[]},
    actionPlans:{critical:false,canLoad:()=>roles('admin_valora','consultor_valora','empresa_admin','gestor_pesquisa','analista_resultados','gestor_area'),load:()=>scopedRows('actionPlans'),fallback:[]},
    supportTickets:{critical:false,canLoad:()=>roles('admin_valora','empresa_admin'),load:()=>scopedRows('supportTickets'),fallback:[]},
    supportMessages:{critical:false,canLoad:()=>roles('admin_valora','empresa_admin'),load:()=>scopedRows('supportMessages'),fallback:[]},
    supportSlaPolicies:{critical:false,canLoad:()=>roles('admin_valora'),load:()=>queryCollection('supportSlaPolicies'),fallback:[]},
    supportCategories:{critical:false,canLoad:()=>roles('admin_valora','empresa_admin'),load:()=>queryCollection('supportCategories'),fallback:[]},
    knowledgeBase:{critical:false,canLoad:()=>p.role!=='convidado_externo',load:()=>queryCollection('knowledgeBase'),fallback:[]},
    notifications:{critical:false,canLoad:()=>p.role!=='convidado_externo',load:()=>p.role==='participante'?queryCollection('notifications',[['userId','==',p.uid||p.id]]):scopedRows('notifications'),fallback:[]},
    settings:{critical:false,canLoad:()=>p.role!=='convidado_externo',load:loadSettings,fallback:{}},
    logs:{critical:false,canLoad:()=>p.role==='admin_valora',load:()=>queryCollection('logs',[],null,300),fallback:[]},
    integrations:{critical:false,canLoad:()=>roles('admin_valora','empresa_admin'),load:()=>scopedRows('integrations'),fallback:[]},
    apiKeys:{critical:false,canLoad:()=>roles('admin_valora','empresa_admin'),load:()=>scopedRows('apiKeys'),fallback:[]},
    webhooks:{critical:false,canLoad:()=>roles('admin_valora','empresa_admin'),load:()=>scopedRows('webhooks'),fallback:[]},
    integrationLogs:{critical:false,canLoad:()=>roles('admin_valora','empresa_admin'),load:()=>scopedRows('integrationLogs'),fallback:[]},
    communications:{critical:false,canLoad:()=>roles('admin_valora','empresa_admin','gestor_pesquisa'),load:()=>scopedRows('communications'),fallback:[]},
    units:{critical:false,canLoad:()=>roles('admin_valora','consultor_valora','empresa_admin','gestor_pesquisa','analista_resultados','gestor_area'),load:()=>scopedRows('units'),fallback:[]},
    serviceDeliverables:{critical:false,canLoad:()=>roles('admin_valora','consultor_valora','empresa_admin'),load:()=>scopedRows('serviceDeliverables'),fallback:[]}
  };
  const keys=Object.keys(manifest);const tasks=keys.map(key=>{const item=manifest[key];if(!item.canLoad()){recordCollectionDebug(key,0,{status:'skipped_by_permission',role:p.role});return Promise.resolve(item.fallback);}return Promise.resolve().then(item.load);});
  const settled=await Promise.allSettled(tasks);const values={};let criticalError=null;
  settled.forEach((result,index)=>{const key=keys[index],item=manifest[key];if(result.status==='fulfilled')values[key]=result.value;else{values[key]=item.fallback;recordFirestoreError(key,result.reason,{operation:'hydrate'});recordCollectionDebug(key,0,{status:'fallback_after_error',role:p.role});if(item.critical&&!criticalError)criticalError=result.reason;}});
  if(criticalError)throw Object.assign(new Error('Não foi possível validar os dados essenciais da sua sessão. Saia, entre novamente e tente outra vez.'),{code:criticalError.code||'critical-hydration-failed'});
...
async function hydrateStore(){
  ...
  catch(err){session.lastError=err;if(session.store){session.store.firestoreLastError={code:err?.code||'',message:err?.message||String(err),at:new Date().toISOString()};session.store.firestoreDebug=firestoreDebug();}console.warn('[Valora Insight] Falha ao carregar Firestore.',err);}
...
async function adminCreateResultShareLink(responseId){if(!responseId)throw Object.assign(new Error('responseId obrigatório.'),{code:'missing_response_id'});const result=await callFunction('adminCreateResultShareLink',{responseId});const data=result?.data||result;if(data?.url&&/resultTokenHash|tokenHash/i.test(data.url))throw Object.assign(new Error('Link de resultado inseguro.'),{code:'unsafe_result_share_url'});return data;}
async function preparePublicSurveyLink(surveyId){const result=await callFunction('preparePublicSurveyLink',{surveyId});return result?.data||result;}
async function adminDeleteResponse(responseId){if(!responseId)throw Object.assign(new Error('responseId obrigatório.'),{code:'missing_response_id'});const result=await callFunction('adminDeleteResponse',{responseId});return result?.data||result;}
async function adminGetResponseResult(responseId){const response=await getDoc('responses',responseId);if(!response)throw Object.assign(new Error('Resposta não encontrada.'),{code:'response_not_found'});const survey=response.surveyId?await getDoc('surveys',response.surveyId).catch(()=>null):null;const companyId=response.companyId||survey?.companyId||survey?.organizationId||'';const company=companyId?await getDoc('organizations',companyId).catch(()=>null)||await getDoc('companies',companyId).catch(()=>null):null;return {ok:true,response,result:response,survey:survey||{},company:company||{},score:{rawScore:response.rawScore,maxScore:response.maxScore,normalized5:response.normalized5,percentage:response.percentage,byDimension:response.byDimension||{}},level:response.level||null};}
...
async function getParticipantResultsByPasswordFirebase(email,password){
  try{return await callFunction('getParticipantResultsByPassword',{email,password});}
  catch(err){throw mapPublicFunctionError(err);}
}
...
window.ValoraFirebaseRepository={
  ...
  adminCreateResultShareLink,
  preparePublicSurveyLink,
  adminGetResponseResult,
  ...
  listForms:loadForms,createForm(data){return callFunction('adminCreateForm',{payload:deepCleanForFirestore(data)});},updateForm(id,data){return callFunction('adminUpdateForm',{id,payload:deepCleanForFirestore(data)});},deleteForm(id){const formId=typeof id==='object'?(id.id||id.formId):id;return callFunction('adminDeleteForm',{id:formId,formId}).then(r=>{invalidateFeaturedHomeSurveyCache();return r;});},adminGetFormUsage(id){const formId=typeof id==='object'?(id.formId||id.id):id;return callFunction('adminGetFormUsage',{id:formId,formId});},adminCloseSurveysByForm(id,mode='close'){return callFunction('adminCloseSurveysByForm',{formId:typeof id==='object'?id.formId:id,mode:typeof id==='object'?(id.mode||mode):mode});},adminCreateForm(data){return callFunction('adminCreateForm',{payload:deepCleanForFirestore(data)});},adminUpdateForm(id,data){const formId=typeof id==='object'?(id.id||id.formId):id;return callFunction('adminUpdateForm',{id:formId,formId,payload:deepCleanForFirestore(data)});},adminDeleteForm(id){const formId=typeof id==='object'?(id.id||id.formId):id;return callFunction('adminDeleteForm',{id:formId,formId}).then(r=>{invalidateFeaturedHomeSurveyCache();return r;});},adminCloneForm(id,changes){return callFunction('adminCloneForm',{formId:id,changes:deepCleanForFirestore(changes||{})});},adminCreateFormVersion(id,changes){return callFunction('adminCreateFormVersion',{formId:id,changes:deepCleanForFirestore(changes||{})});},repairOfficialFormDocument(){return callFunction('repairOfficialFormDocument',{});},
  listSurveys:loadSurveys,createSurvey(data){return callFunction('adminCreateSurvey',{payload:deepCleanForFirestore(data)});},updateSurvey(id,data){return callFunction('adminUpdateSurvey',{id,payload:deepCleanForFirestore(data)});},deleteSurvey(id){return callFunction('adminDeleteSurvey',{id}).then(r=>{invalidateFeaturedHomeSurveyCache();return r;});},adminCreateSurvey(data){return callFunction('adminCreateSurvey',{payload:deepCleanForFirestore(data)});},adminUpdateSurvey(id,data){return callFunction('adminUpdateSurvey',{id,payload:deepCleanForFirestore(data)});},adminDeleteSurvey(id){return callFunction('adminDeleteSurvey',{id}).then(r=>{invalidateFeaturedHomeSurveyCache();return r;});},adminArchiveSurvey(id){return callFunction('adminArchiveSurvey',{id}).then(r=>{invalidateFeaturedHomeSurveyCache();return r;});},adminCloseSurvey(id){return callFunction('adminCloseSurvey',{id}).then(r=>{invalidateFeaturedHomeSurveyCache();return r;});},
  listResponses:loadResponses,getResponse(id){return getDoc('responses',id);},adminDeleteResponse,
  ...
  listActionPlans(companyId,filters={}){const fs=buildScopeFilter(companyId);Object.entries(filters||{}).forEach(([k,v])=>{if(v)fs.push([k,'==',v]);});return queryCollection('actionPlans',fs);},createActionPlan(data){return callFunction('createActionPlan',{payload:deepCleanForFirestore(data)});},updateActionPlan(id,data){return updateDoc('actionPlans',id,data);},deleteActionPlan(id){return deleteDoc('actionPlans',id);},addActionComment(actionId,comment){return updateDoc('actionPlans',actionId,{comments:fv().arrayUnion(comment)});},markActionCompleted(actionId){return updateDoc('actionPlans',actionId,{status:'completed',progress:100,completedAt:ts()});},generateActionPlansFromSurvey(surveyId){return queryCollection('actionPlans',[["surveyId","==",surveyId]]);},
