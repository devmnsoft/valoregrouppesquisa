...
- Branch local: `work`; HEAD inicial `11f49e6` (merge do PR #552).
- O checkout não possui remoto configurado. Portanto não foi possível executar `fetch`, comparar commits posteriores, fazer `push`, abrir PR remoto ou integrar a branch-base.
...
**Implementado:** o helper passou a receber explicitamente `object parameters`. `MyDayAsync` envia `{ o, u, wide }`; recentes e fixados enviam somente `{ o, u }`. Não foi criada coluna, removido filtro, concatenado booleano nem convertido erro em lista vazia. O predicado de organização permanece obrigatório. Itens sem proprietário continuam compartilhados porque essa é a regra já expressa pelo SQL vigente.

**Implementado:** a API continua derivando a visão ampla exclusivamente dos papéis autenticados `admin_valora` e `admin_cliente`; não existe parâmetro HTTP `wide` controlado pelo navegador.

### Evidência de teste

- **Implementado, não executado localmente:** teste PostgreSQL exercita visão restrita e ampla, itens próprios/de terceiros/sem proprietário, isolamento entre duas organizações, recentes e pin/unpin repetidos. O teste recusa nomes de banco sem marcador `test`, `teste`, `homolog` ou `qa` e remove apenas os UUIDs aleatórios criados por ele.
...
1. **P0:** disponibilizar .NET SDK 10 e PostgreSQL descartável migrado; executar a regressão e acrescentar teste de host autenticado que prove que um usuário comum não consegue solicitar visão ampla.
2. **P0:** configurar remoto/autorização para comparar a branch-base, fazer `fetch`/`push` e abrir PR. Não há evidência local de commits posteriores a `11f49e6`.
