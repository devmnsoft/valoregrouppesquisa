...
public sealed class EnterpriseRepository(IDbConnectionFactory connections) : IEnterpriseRepository {
    ...
    public async Task<Guid> CreateLeadAsync(CrmLead lead, CancellationToken ct) { const string sql = "INSERT INTO valorapesquisa.crm_leads(id,name,company_name,email,phone,commercial_status,intended_plan,owner_name,next_action_at,notes) VALUES(@Id,@Name,@CompanyName,@Email,@Phone,@Status,@IntendedPlan,@Owner,@NextActionAt,@Notes) RETURNING id"; using var c = connections.Create(); return await c.ExecuteScalarAsync<Guid>(new CommandDefinition(sql, lead, cancellationToken: ct)); }

    public async Task UpdateCompanyStatusAsync(Guid id, string status, CancellationToken ct) { using var c = connections.Create(); var changed = await c.ExecuteAsync(new CommandDefinition("UPDATE valorapesquisa.organizations SET account_status=@status,updated_at=now() WHERE id=@id AND deleted_at IS NULL", new { id, status }, cancellationToken: ct)); if (changed == 0) throw new KeyNotFoundException("Empresa não encontrada."); }

    public async Task<IReadOnlyList<EnterpriseItem>> ListItemsAsync(Guid? organizationId, string kind, CancellationToken ct) { using var c = connections.Create(); var rows = await c.QueryAsync<EnterpriseRow>(new CommandDefinition("SELECT id,organization_id OrganizationId,kind,name,status,configuration::text Configuration,created_at CreatedAt,updated_at UpdatedAt FROM valorapesquisa.enterprise_items WHERE kind=@kind AND deleted_at IS NULL AND (@organizationId::uuid IS NULL OR organization_id=@organizationId::uuid) ORDER BY name", new { organizationId, kind }, cancellationToken: ct)); return rows.Select(Map).ToList(); }

    public async Task<Guid> UpsertItemAsync(Guid? organizationId, Guid? id, UpsertEnterpriseItemRequest request, CancellationToken ct) { const string sql = """INSERT INTO valorapesquisa.enterprise_items(id,organization_id,kind,name,status,configuration) VALUES(COALESCE(@id,gen_random_uuid()),@organizationId,@Kind,@Name,@Status,CAST(@Configuration AS jsonb)) ON CONFLICT(id) DO UPDATE SET name=EXCLUDED.name,status=EXCLUDED.status,configuration=EXCLUDED.configuration,updated_at=now() WHERE enterprise_items.organization_id IS NOT DISTINCT FROM @organizationId RETURNING id"""; using var c = connections.Create(); return await c.ExecuteScalarAsync<Guid>(new CommandDefinition(sql, new { id, organizationId, request.Kind, request.Name, request.Status, Configuration = request.Configuration.GetRawText() }, cancellationToken: ct)); }

    public async Task SetItemStatusAsync(Guid? organizationId, Guid id, string status, CancellationToken ct) { using var c = connections.Create(); await c.ExecuteAsync(new CommandDefinition("UPDATE valorapesquisa.enterprise_items SET status=@status,updated_at=now() WHERE id=@id AND organization_id IS NOT DISTINCT FROM @organizationId AND deleted_at IS NULL", new { organizationId, id, status }, cancellationToken: ct)); }

    public async Task<Guid> CreateApiKeyAsync(Guid organizationId, string name, IReadOnlyList<string> scopes, string hash, string prefix, DateTime? expiresAt, Guid createdBy, CancellationToken ct) { const string sql = "INSERT INTO valorapesquisa.api_keys(organization_id,name,key_prefix,key_hash,scopes,status,expires_at,created_by) VALUES(@organizationId,@name,@prefix,@hash,@scopes,'active',@expiresAt,@createdBy) RETURNING id"; using var c = connections.Create(); return await c.ExecuteScalarAsync<Guid>(new CommandDefinition(sql, new { organizationId, name, prefix, hash, scopes = scopes.ToArray(), expiresAt, createdBy }, cancellationToken: ct)); }

    public async Task<IReadOnlyList<ApiKeySummary>> ListApiKeysAsync(Guid organizationId, CancellationToken ct) { const string sql = "SELECT id Id,name Name,key_prefix Prefix,scopes Scopes,CASE WHEN revoked_at IS NOT NULL THEN 'revoked' WHEN expires_at IS NOT NULL AND expires_at<=now() THEN 'expired' ELSE 'active' END Status,created_at CreatedAt,expires_at ExpiresAt,last_used_at LastUsedAt FROM valorapesquisa.api_keys WHERE organization_id=@organizationId AND deleted_at IS NULL ORDER BY created_at DESC"; using var c = connections.Create(); return (await c.QueryAsync<ApiKeySummary>(new CommandDefinition(sql, new { organizationId }, cancellationToken: ct))).ToList(); }

    public async Task<ApiKeySummary?> GetApiKeyAsync(Guid organizationId, Guid id, CancellationToken ct) { const string sql = "SELECT id Id,name Name,key_prefix Prefix,scopes Scopes,CASE WHEN revoked_at IS NOT NULL THEN 'revoked' WHEN expires_at IS NOT NULL AND expires_at<=now() THEN 'expired' ELSE status END Status,created_at CreatedAt,expires_at ExpiresAt,last_used_at LastUsedAt FROM valorapesquisa.api_keys WHERE organization_id=@organizationId AND id=@id AND deleted_at IS NULL"; using var c = connections.Create(); return await c.QuerySingleOrDefaultAsync<ApiKeySummary>(new CommandDefinition(sql, new { organizationId, id }, cancellationToken: ct)); }

    public async Task<IReadOnlyList<ApiKeyUsage>> ListApiKeyUsageAsync(Guid organizationId, Guid id, CancellationToken ct) { const string sql = "SELECT id Id,endpoint Endpoint,method Method,status_code StatusCode,scope_used ScopeUsed,correlation_id CorrelationId,created_at CreatedAt FROM valorapesquisa.api_key_usage_events WHERE organization_id=@organizationId AND api_key_id=@id AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 100"; using var c = connections.Create(); return (await c.QueryAsync<ApiKeyUsage>(new CommandDefinition(sql, new { organizationId, id }, cancellationToken: ct))).ToList(); }

    public async Task<bool> RevokeApiKeyAsync(Guid organizationId, Guid id, CancellationToken ct) { const string sql = "UPDATE valorapesquisa.api_keys SET revoked_at=now(),status='revoked',updated_at=now() WHERE id=@id AND organization_id=@organizationId AND revoked_at IS NULL AND deleted_at IS NULL"; using var c = connections.Create(); return await c.ExecuteAsync(new CommandDefinition(sql, new { organizationId, id }, cancellationToken: ct)) == 1; }
