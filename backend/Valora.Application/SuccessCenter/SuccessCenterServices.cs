...
public interface ISuccessCenterRepository {
    ...
    Task<IReadOnlyList<SupportTicket>> GetTickets(Guid organizationId, CancellationToken ct); Task<SupportTicketDetails?> GetTicket(Guid organizationId, Guid ticketId, bool includeInternal, CancellationToken ct); Task<Guid> CreateTicket(Guid organizationId, Guid userId, CreateTicketCommand command, CancellationToken ct);
