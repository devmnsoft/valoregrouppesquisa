...
const missing=["plan_upgrade_requests","requestPlanUpgrade","currentPlanCode","targetPlanCode","upgrade-requests"].filter(x=>!corpus.includes(x));
if(missing.length){console.error('Missing legacy validation markers for upgrade-requests:',missing.join(', '));process.exit(1);}
...
console.log('OK legacy upgrade-requests');
