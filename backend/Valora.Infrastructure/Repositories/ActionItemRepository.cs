...
public sealed class ActionItemRepository(IDbConnectionFactory db, IDbTransactionFactory tx) : IActionItemRepository {
    ...
    private async Task Modify(Guid o,Guid u,Guid id,bool wide,long version,string commandId,string operation,object request,string? reason,CancellationToken c) {
        ...
        var current=await z.Connection.QuerySingleOrDefaultAsync<EditableItem>(new CommandDefinition("SELECT i.status,i.version,i.title,i.description,i.priority,i.expected_outcome ExpectedOutcome,i.responsible_user_id ResponsibleUserId,i.due_at DueAt,old_user.name ResponsibleName FROM valorapesquisa.action_items i JOIN valorapesquisa.action_plans p ON p.id=i.action_plan_id AND p.organization_id=i.organization_id LEFT JOIN valorapesquisa.users old_user ON old_user.id=i.responsible_user_id AND old_user.organization_id=i.organization_id WHERE i.id=@id AND i.organization_id=@o AND i.deleted_at IS NULL AND p.deleted_at IS NULL AND (@wide OR i.responsible_user_id IS NULL OR i.responsible_user_id=@u OR p.owner_user_id=@u) FOR UPDATE OF i",new{o,u,id,wide},z.Transaction,cancellationToken:c));
