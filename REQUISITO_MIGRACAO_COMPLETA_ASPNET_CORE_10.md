...
5. a criação do banco canônico em `database/postgresql/banco_completo.sql`;
...
```text
database/postgresql/banco_completo.sql
```
...
10. `database/postgresql/banco_completo.sql` idempotente.
...
Não crie `backend-v3`, outra solution, React, Angular, Vue, Vite, SPA paralela ou novo produto. `backend-v2` e o legado Firebase da raiz são somente fontes de comparação e não devem receber novas funcionalidades. A versão final de produção deve executar apenas `Valora.Web`, `Valora.Api`, `Valora.Worker` e PostgreSQL, sem Firebase no runtime.

### Antes de alterar

1. Leia este documento inteiro.
2. Faça inventário da raiz, `functions`, regras Firestore, repositories JavaScript, serviços JavaScript, `backend`, `backend-v2`, banco, testes, scripts e documentação.
...
1. Crie `database/postgresql/banco_completo.sql` como fonte canônica.
