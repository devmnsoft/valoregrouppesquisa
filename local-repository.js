...
window.ValoraLocalRepository={
  ...
  updateOrganizationBrand(id,brand,{state}={}){const item=(state?.companies||[]).find(x=>x.id===id);if(item)item.brand={...(item.brand||{}),...brand};return this.updateOrganization(id,{brand:item?.brand},{state});},
  updateOrganizationSubscription(id,subscription,{state}={}){const item=(state?.companies||[]).find(x=>x.id===id);if(item)item.subscription={...(item.subscription||{}),...subscription};return this.updateOrganization(id,{subscription:item?.subscription},{state});},
  updateOrganizationSettings(id,settings,{state}={}){const item=(state?.companies||[]).find(x=>x.id===id);if(item)item.settings={...(item.settings||{}),...settings};return this.updateOrganization(id,{settings:item?.settings},{state});},
  updateOrganizationLimits(id,limits,{state}={}){const item=(state?.companies||[]).find(x=>x.id===id);if(item)item.limitsOverride={...(item.limitsOverride||{}),...limits};return this.updateOrganization(id,{limitsOverride:item?.limitsOverride},{state});},
  getOrganizationBySlug(slug,{state}={}){return Promise.resolve(clone((state?.companies||[]).find(x=>x.slug===slug)||null));},
  checkSlugAvailability(slug,{state,currentId=''}={}){return Promise.resolve(!(state?.companies||[]).some(x=>x.id!==currentId&&x.slug===slug));},
