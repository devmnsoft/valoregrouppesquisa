...
Esta sprint corrigiu a rota arquitetural para declarar `backend/Valora.sln` como base oficial da nova versão .NET, preservando `projeto .NET predecessor removido` apenas como referência temporária.
...
## 3. Reaproveitado de `projeto .NET predecessor removido`
...
Não foram portados a solution paralela, namespaces `ValoraPesquisa.*`, novo frontend, Dockerfiles paralelos como base oficial, scripts de build oficiais apontando para `projeto .NET predecessor removido` ou seeds inseguros.
...
Riscos principais: divergência SQL, endpoints duplicados, Web consumindo fallback, exposição sensível em logs/DTOs e automações oficiais ainda chamando `projeto .NET predecessor removido`.
