...
public sealed class UserAdministrationService(IUserAdministrationRepository repository) : IUserAdministrationService {
    ...
    public async Task UpdateStatusAsync(Guid org, Guid actor, Guid id, UpdateUserStatusRequest r, CancellationToken ct = default) { Tenant(org); if (r.Status is not ("active" or "inactive" or "suspended" or "locked")) throw new ValidationAppException("Status inválido."); if (r.Status != "active" && await repository.IsAdministratorAsync(org, id, ct) && await repository.CountAdministratorsAsync(org, ct) <= 1) throw new BusinessRuleAppException("LAST_ADMINISTRATOR: O último administrador não pode ser desativado ou suspenso."); if (!await repository.UpdateStatusAsync(org, id, r.Status, ct)) throw new NotFoundAppException("Usuário não encontrado."); }
