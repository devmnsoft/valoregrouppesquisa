...
public sealed class PlanEntitlementService(IPlanRepository plans, ISurveyRepository surveys, IOrganizationRepository orgs, ILogger<PlanEntitlementService> logger): IPlanEntitlementService{
 public async Task<PlanEntitlements> ResolveAsync(Guid organizationId){ logger.LogInformation("Plan entitlement resolve started. OrganizationId={OrganizationId}", organizationId); var id=await plans.GetCurrentPlanIdAsync(organizationId)??"free"; var p=await plans.GetByIdAsync(id)??await plans.GetByIdAsync("free"); return new(id,p?.Limits??new(),p?.Capabilities??new()); }
