    ...
    IAccessAdministrationService? accessAdministration = null)
{
    public async Task<AuthenticationResult> RegisterCompanyAsync(RegisterCompanyRequest request)
    {
        ...
        var selectedPlanCode = request.PlanCode.Trim().ToLowerInvariant() is "start" or "growth" ? request.PlanCode.Trim().ToLowerInvariant() : "free";
        var selectedPlan = await plans.GetByIdAsync(selectedPlanCode);
        if (selectedPlan is null)
            throw new ApplicationConfigurationException($"Required system plan '{selectedPlanCode}' was not found.");
