...
A versão canônica desta entrega é **8.6.1** (`APP_VERSION` em `config.js`). O Firebase Hosting usa `firebase.json` para enviar HTML com `Cache-Control: no-store` e assets JS/CSS/imagens com cache longo, desde que os assets continuem versionados no `index.html` com `?v=8.6.1`.
