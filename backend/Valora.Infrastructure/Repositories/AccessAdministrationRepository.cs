...
public sealed class AccessAdministrationRepository(Application.Contracts.IDbConnectionFactory factory) : IAccessAdministrationRepository
{
    ...
    public async Task<EffectiveAccessDto?> GetEffectiveAccessAsync(Guid org, Guid user, CancellationToken ct)
    {
        ...
        var candidate=(await c.QueryAsync<PermissionState>(new CommandDefinition("SELECT DISTINCT p.code Code,p.module_code ModuleCode,(r.organization_id IS NULL AND lower(r.code)=@platformRole) IsPlatform FROM valorapesquisa.user_roles ur JOIN valorapesquisa.roles r ON r.id=ur.role_id AND r.deleted_at IS NULL AND r.status='active' JOIN valorapesquisa.role_permissions rp ON rp.role_id=r.id JOIN valorapesquisa.permissions p ON p.id=rp.permission_id AND p.status='active' WHERE ur.user_id=@user AND (r.organization_id IS NULL OR r.organization_id=@org)",new{org,user,platformRole=ValoraAccessCatalog.PlatformRole},cancellationToken:ct))).AsList();
        var enabled=status.UserStatus=="active"&&status.OrganizationStatus=="active"&&status.SubscriptionStatus is "active" or "trialing";
        var isPlatform=candidate.Any(p=>p.IsPlatform);
        var availableModules=(isPlatform?ValoraAccessCatalog.PlatformModules:modules).Distinct(StringComparer.OrdinalIgnoreCase).ToArray();
        var granted=candidate.Where(p=>p.IsPlatform||(enabled&&p.ModuleCode is not null&&modules.Contains(p.ModuleCode))).Select(p=>p.Code).Distinct(StringComparer.OrdinalIgnoreCase).Order().ToArray();
        var blocked=candidate.Where(p=>!granted.Contains(p.Code,StringComparer.OrdinalIgnoreCase)).Select(p=>new BlockedAccessDto(p.Code,!enabled?"Conta, organização ou assinatura inativa.":p.ModuleCode is null?"Permissão legada aguardando classificação.":"Módulo indisponível no plano ou na organização.")).ToArray();
        var scopes=(await c.QueryAsync<AccessScopeDto>(new CommandDefinition("SELECT scope_type Type,scope_id Id,scope_type||' · '||scope_id::text Label FROM valorapesquisa.user_scopes WHERE organization_id=@org AND user_id=@user AND deleted_at IS NULL ORDER BY scope_type",new{org,user},cancellationToken:ct))).AsList(); return new(user,granted,blocked,scopes,availableModules);
    }
    ...
    private sealed record RoleLock(Guid Id,bool IsSystem,long Version); private sealed record PermissionState(string Code,string? ModuleCode,bool IsPlatform); private sealed record UserState(string UserStatus,string OrganizationStatus,string SubscriptionStatus);
}
