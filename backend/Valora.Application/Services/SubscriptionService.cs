...
public sealed class SubscriptionService(ISubscriptionRepository repo) : ISubscriptionService {
    ...
    public async Task UpdateAsync(Guid organizationId, UpdateSubscriptionRequest request) {
        ...
        await repo.UpsertAsync(new SubscriptionDto {
            Id = Guid.NewGuid(),
            OrganizationId = organizationId,
            PlanId = request.PlanId,
            Status = request.Status,
            BillingCycle = request.BillingCycle,
            ContractedValue = request.ContractedValue,
            DiscountValue = request.DiscountValue,
            StartsAt = request.StartsAt,
            RenewalAt = request.RenewalAt,
            DueAt = request.DueAt,
            FinancialContact = request.FinancialContact,
            FinancialEmail = request.FinancialEmail,
            FinancialPhone = request.FinancialPhone,
