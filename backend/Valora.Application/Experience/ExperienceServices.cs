...
public sealed class GeneratePublicResultViewUseCase(IPublicResultViewRepository repository) {
    public async Task<IssuedPublicToken> ExecuteAsync(Guid organizationId, Guid diagnosticId, Guid resultId, string title, DateTimeOffset expiresAt, bool report, bool certificate, CancellationToken ct = default) { var raw = ExperienceToken.Generate(); var id = await repository.CreateAsync(organizationId, diagnosticId, resultId, title, ExperienceToken.Hash(raw), expiresAt, report, certificate, ct); return new(id, raw, expiresAt); }
}
