...
-- Valora Success Center™: tenant-isolated, evidence-first customer success
CREATE TABLE IF NOT EXISTS valorapesquisa.success_onboarding_flows(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),code text NOT NULL UNIQUE,name text NOT NULL,description text NOT NULL,status text NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.success_onboarding_steps(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),flow_id uuid NOT NULL REFERENCES valorapesquisa.success_onboarding_flows(id),code text NOT NULL,title text NOT NULL,executive_guidance text NOT NULL,action_url text NOT NULL,display_order int NOT NULL,suggested_days int NOT NULL DEFAULT 7,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(flow_id,code));
CREATE TABLE IF NOT EXISTS valorapesquisa.organization_onboarding_progress(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),step_id uuid NOT NULL REFERENCES valorapesquisa.success_onboarding_steps(id),status text NOT NULL DEFAULT 'pending',responsible_user_id uuid REFERENCES valorapesquisa.users(id),due_at timestamptz,evidence text,blocked_reason text,completed_by_user_id uuid REFERENCES valorapesquisa.users(id),completed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(organization_id,step_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.organization_success_profiles(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),lifecycle_stage text NOT NULL DEFAULT 'onboarding',onboarding_status text NOT NULL DEFAULT 'pending',success_manager_user_id uuid REFERENCES valorapesquisa.users(id),primary_contact_user_id uuid REFERENCES valorapesquisa.users(id),implementation_goal text,expected_go_live_at timestamptz,actual_go_live_at timestamptz,risk_level text NOT NULL DEFAULT 'unknown',risk_reason text,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE UNIQUE INDEX IF NOT EXISTS ux_success_profile_org ON valorapesquisa.organization_success_profiles(organization_id) WHERE deleted_at IS NULL;
CREATE TABLE IF NOT EXISTS valorapesquisa.organization_health_scores(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),score numeric(5,2) NOT NULL CHECK(score BETWEEN 0 AND 100),level text NOT NULL,usage_score numeric(5,2) NOT NULL,adoption_score numeric(5,2) NOT NULL,support_score numeric(5,2) NOT NULL,diagnostic_score numeric(5,2) NOT NULL,engagement_score numeric(5,2) NOT NULL,risk_level text NOT NULL,evidence_summary text NOT NULL,calculated_at timestamptz NOT NULL,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_health_org_calculated ON valorapesquisa.organization_health_scores(organization_id,calculated_at DESC) WHERE deleted_at IS NULL;
CREATE TABLE IF NOT EXISTS valorapesquisa.success_playbooks(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),code text NOT NULL UNIQUE,name text NOT NULL,description text NOT NULL,expected_result text NOT NULL,status text NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.success_playbook_tasks(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),playbook_id uuid NOT NULL REFERENCES valorapesquisa.success_playbooks(id),title text NOT NULL,guidance text NOT NULL,responsible_role text NOT NULL,suggested_days int NOT NULL,evidence_required boolean NOT NULL DEFAULT true,expected_result text NOT NULL,display_order int NOT NULL,status text NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.organization_playbook_assignments(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),playbook_id uuid NOT NULL REFERENCES valorapesquisa.success_playbooks(id),assigned_by_user_id uuid REFERENCES valorapesquisa.users(id),status text NOT NULL DEFAULT 'active',assigned_at timestamptz NOT NULL DEFAULT now(),completed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE UNIQUE INDEX IF NOT EXISTS ux_org_playbook_active ON valorapesquisa.organization_playbook_assignments(organization_id,playbook_id) WHERE deleted_at IS NULL;
ALTER TABLE valorapesquisa.support_tickets ADD COLUMN IF NOT EXISTS opened_by_user_id uuid REFERENCES valorapesquisa.users(id),ADD COLUMN IF NOT EXISTS assigned_to_user_id uuid REFERENCES valorapesquisa.users(id),ADD COLUMN IF NOT EXISTS category text NOT NULL DEFAULT 'duvida_operacional',ADD COLUMN IF NOT EXISTS source text NOT NULL DEFAULT 'portal';
UPDATE valorapesquisa.support_tickets SET opened_by_user_id=user_id WHERE opened_by_user_id IS NULL AND user_id IS NOT NULL;
CREATE TABLE IF NOT EXISTS valorapesquisa.support_ticket_messages(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),ticket_id uuid NOT NULL REFERENCES valorapesquisa.support_tickets(id),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),message text NOT NULL,context_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.support_ticket_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),ticket_id uuid NOT NULL REFERENCES valorapesquisa.support_tickets(id),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),user_id uuid REFERENCES valorapesquisa.users(id),event_type text NOT NULL,from_status text,to_status text,evidence text,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_base_categories(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),code text NOT NULL UNIQUE,name text NOT NULL,description text NOT NULL,display_order int NOT NULL DEFAULT 0,status text NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_base_articles(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),category_id uuid REFERENCES valorapesquisa.knowledge_base_categories(id),slug text NOT NULL UNIQUE,title text NOT NULL,summary text NOT NULL,content text NOT NULL,visibility text NOT NULL DEFAULT 'public',minimum_plan_code text,status text NOT NULL DEFAULT 'draft',display_order int NOT NULL DEFAULT 0,search_document tsvector GENERATED ALWAYS AS (to_tsvector('portuguese',coalesce(title,'')||' '||coalesce(summary,'')||' '||coalesce(content,''))) STORED,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_kb_article_search ON valorapesquisa.knowledge_base_articles USING gin(search_document);
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_base_article_feedback(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),article_id uuid NOT NULL REFERENCES valorapesquisa.knowledge_base_articles(id),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),user_id uuid REFERENCES valorapesquisa.users(id),helpful boolean NOT NULL,comment text,support_ticket_id uuid REFERENCES valorapesquisa.support_tickets(id),created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(article_id,organization_id,user_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.product_usage_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),user_id uuid REFERENCES valorapesquisa.users(id),event_name text NOT NULL,feature_code text NOT NULL,context_json jsonb NOT NULL DEFAULT '{}'::jsonb,occurred_at timestamptz NOT NULL DEFAULT now(),created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_product_usage_org_feature ON valorapesquisa.product_usage_events(organization_id,feature_code,occurred_at DESC);
CREATE TABLE IF NOT EXISTS valorapesquisa.feature_adoption_metrics(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),feature_code text NOT NULL,period_start date NOT NULL,period_end date NOT NULL,event_count int NOT NULL DEFAULT 0,active_user_count int NOT NULL DEFAULT 0,adoption_rate numeric(5,2),evidence_summary text NOT NULL,calculated_at timestamptz NOT NULL DEFAULT now(),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(organization_id,feature_code,period_start,period_end));
CREATE TABLE IF NOT EXISTS valorapesquisa.customer_success_notes(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),author_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),note_type text NOT NULL,content text NOT NULL,evidence_reference text,follow_up_at timestamptz,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);

INSERT INTO valorapesquisa.success_onboarding_flows(code,name,description) VALUES('default','Implantação Valora Insight','Jornada guiada para ativação responsável e comprovável.') ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name,description=EXCLUDED.description,updated_at=now();
INSERT INTO valorapesquisa.success_onboarding_steps(flow_id,code,title,executive_guidance,action_url,display_order,suggested_days)
SELECT f.id,v.code,v.title,v.guidance,v.url,v.ord,v.days FROM valorapesquisa.success_onboarding_flows f CROSS JOIN (VALUES
('organization_profile','Completar dados da organização','Confirme o contexto institucional antes de interpretar resultados.','/Organization#org-profile',1,2),('units','Cadastrar unidades','Modele a estrutura real para preservar comparabilidade.','/Organization#org-structure',2,3),('users','Convidar usuários','Convide somente responsáveis com necessidade de acesso.','/Users',3,4),('permissions','Configurar permissões','Aplique o menor privilégio e preserve a decisão humana.','/Users/Permissions',4,5),('methodology','Escolher metodologia ativa','Valide a metodologia adequada ao objetivo de implantação.','/Methodology',5,6),('first_diagnostic','Criar primeiro diagnóstico','Defina objetivo, escopo e evidências antes da coleta.','/Diagnostics/New',6,7),('invitations','Enviar convites','Revise público e privacidade antes do disparo.','/Diagnostics',7,8),('responses','Acompanhar respostas','Monitore adesão sem expor identidades indevidas.','/Responses',8,12),('first_result','Gerar primeiro resultado','Calcule somente com amostra e evidências suficientes.','/Results',9,14),('ai_review','Revisar insights da IA','A IA apoia; a validação e a decisão permanecem humanas.','/Insights',10,15),('executive_report','Gerar relatório executivo','Comunique conclusões, limites e fontes de evidência.','/Reports',11,16),('action_plan','Criar plano de ação','Converta decisão validada em responsáveis e prazos.','/ActionCenter',12,18),('evolution','Acompanhar evolução','Compare ciclos equivalentes e registre aprendizados.','/Evolution',13,30)) v(code,title,guidance,url,ord,days) WHERE f.code='default' ON CONFLICT(flow_id,code) DO UPDATE SET title=EXCLUDED.title,executive_guidance=EXCLUDED.executive_guidance,action_url=EXCLUDED.action_url,display_order=EXCLUDED.display_order,suggested_days=EXCLUDED.suggested_days;
CREATE OR REPLACE FUNCTION valorapesquisa.ensure_success_onboarding(p_organization_id uuid) RETURNS void LANGUAGE sql AS $$ INSERT INTO valorapesquisa.organization_onboarding_progress(organization_id,step_id,due_at) SELECT p_organization_id,s.id,now()+(s.suggested_days||' days')::interval FROM valorapesquisa.success_onboarding_steps s JOIN valorapesquisa.success_onboarding_flows f ON f.id=s.flow_id AND f.code='default' WHERE s.deleted_at IS NULL ON CONFLICT(organization_id,step_id) DO NOTHING $$;
ALTER TABLE valorapesquisa.success_onboarding_steps ADD COLUMN IF NOT EXISTS is_required boolean NOT NULL DEFAULT true;
CREATE TABLE IF NOT EXISTS valorapesquisa.onboarding_flows(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),code text NOT NULL UNIQUE,name text NOT NULL,status text NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now());
INSERT INTO valorapesquisa.onboarding_flows(code,name) VALUES('default','Implantação Valora Insight') ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name,updated_at=now();
CREATE TABLE IF NOT EXISTS valorapesquisa.onboarding_step_completions(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),progress_id uuid NOT NULL REFERENCES valorapesquisa.organization_onboarding_progress(id),step_id uuid NOT NULL,user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),status text NOT NULL,evidence text,completed_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_onboarding_completions_org ON valorapesquisa.onboarding_step_completions(organization_id,completed_at DESC);
CREATE TABLE IF NOT EXISTS valorapesquisa.customer_adoption_metrics(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),feature_code text NOT NULL,event_count bigint NOT NULL DEFAULT 0,active_users int NOT NULL DEFAULT 0,adoption_rate numeric(5,2) NOT NULL DEFAULT 0,measured_at timestamptz NOT NULL DEFAULT now(),UNIQUE(organization_id,feature_code,measured_at));
CREATE TABLE IF NOT EXISTS valorapesquisa.customer_health_scores(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),score numeric(5,2) NOT NULL CHECK(score BETWEEN 0 AND 100),level text NOT NULL,usage_score numeric(5,2) NOT NULL,adoption_score numeric(5,2) NOT NULL,diagnostic_score numeric(5,2) NOT NULL,engagement_score numeric(5,2) NOT NULL,risk_level text NOT NULL,calculated_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_customer_health_org ON valorapesquisa.customer_health_scores(organization_id,calculated_at DESC);
...
('customer_success.view','Visualizar Customer Success','Consulta saúde e implantação dos clientes.','operations'),
('customer_success.manage','Gerenciar Customer Success','Gerencia a operação de sucesso do cliente.','operations'),
('customer_success.notes.manage','Gerenciar notas de sucesso','Mantém notas internas auditáveis.','operations'),
('customer_success.tasks.manage','Gerenciar tarefas de sucesso','Mantém tarefas de acompanhamento.','operations'),
('customer_health_score.view','Visualizar health score','Consulta o score e seus componentes.','operations')
ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name,description=EXCLUDED.description,module_code=EXCLUDED.module_code,updated_at=now();
INSERT INTO valorapesquisa.knowledge_base_categories(code,name,description,display_order) VALUES('getting-started','Primeiros passos','Orientações para ativar sua operação.',1),('analysis','Análise e resultados','Leitura responsável de evidências.',2),('support','Suporte','Ajuda e rastreabilidade.',3) ON CONFLICT(code) DO UPDATE SET name=EXCLUDED.name,description=EXCLUDED.description;
INSERT INTO valorapesquisa.knowledge_base_articles(category_id,slug,title,summary,content,visibility,status,display_order)
SELECT c.id,v.slug,v.title,v.summary,v.content,'public','published',v.ord FROM valorapesquisa.knowledge_base_categories c CROSS JOIN (VALUES
('primeiro-diagnostico','Como criar o primeiro diagnóstico','Prepare objetivo, escopo e público.','Acesse Diagnósticos, escolha a metodologia ativa, documente o objetivo e revise o público antes de publicar.',1),('convidar-respondentes','Como convidar respondentes','Envie convites com privacidade.','Revise o público autorizado, o prazo e o texto antes do envio. Acompanhe apenas indicadores agregados.',2),('interpretar-dashboard','Como interpretar o dashboard','Leia indicadores com contexto.','Consulte período, amostra e evidências. Indicadores apoiam a decisão humana e não substituem validação.',3),('relatorio-executivo','Como gerar relatório executivo','Produza uma síntese rastreável.','Selecione um resultado calculado, revise evidências e limitações e então gere o relatório.',4),('action-center','Como usar o Action Center','Transforme decisões em execução.','Crie ações somente após validação, atribua responsável, prazo, evidência e acompanhe check-ins.',5),('certificados','Como acessar certificados','Emita e valide documentos autorizados.','Acesse Certificados, selecione o ciclo autorizado e use a validação pública quando aplicável.',6),('ia-valora','Como funciona a IA Valora','IA com evidência e revisão humana.','A IA Valora usa o contexto autorizado, registra evidências e exige revisão humana antes da publicação.',7),('abrir-chamado','Como abrir chamado de suporte','Registre contexto e acompanhe o histórico.','Acesse Sucesso do Cliente > Chamados, escolha categoria e prioridade e descreva o contexto sem dados pessoais desnecessários.',8)) v(slug,title,summary,content,ord) WHERE c.code=CASE WHEN v.slug='abrir-chamado' THEN 'support' WHEN v.ord>2 THEN 'analysis' ELSE 'getting-started' END ON CONFLICT(slug) DO UPDATE SET title=EXCLUDED.title,summary=EXCLUDED.summary,content=EXCLUDED.content,status='published',updated_at=now();
...
-- 2026-08-27 · Valora Governance & Execution Layer™
...
CREATE TABLE IF NOT EXISTS valorapesquisa.decision_evidence_links (decision_id uuid NOT NULL REFERENCES valorapesquisa.governance_decisions(id), evidence_id uuid NOT NULL, evidence_type varchar(50) NOT NULL, rationale text NOT NULL, linked_by_user_id uuid REFERENCES valorapesquisa.users(id), linked_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY(decision_id,evidence_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.action_item_evidence_links (action_item_id uuid NOT NULL REFERENCES valorapesquisa.action_items(id), evidence_id uuid NOT NULL, evidence_type varchar(50) NOT NULL, purpose varchar(30) NOT NULL DEFAULT 'support', linked_by_user_id uuid REFERENCES valorapesquisa.users(id), linked_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY(action_item_id,evidence_id,purpose));
CREATE TABLE IF NOT EXISTS valorapesquisa.action_item_updates (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id), action_item_id uuid NOT NULL REFERENCES valorapesquisa.action_items(id), status varchar(30) NOT NULL, progress_percent integer NOT NULL CHECK(progress_percent BETWEEN 0 AND 100), note text NOT NULL, evidence_id uuid, created_by_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id), created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.evolution_milestones (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id), evolution_cycle_id uuid NOT NULL REFERENCES valorapesquisa.evolution_cycles(id), title text NOT NULL, description text NOT NULL DEFAULT '', target_at timestamptz, reached_at timestamptz, evidence_summary text NOT NULL, created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.evolution_metric_snapshots (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id), evolution_cycle_id uuid NOT NULL REFERENCES valorapesquisa.evolution_cycles(id), metric_name text NOT NULL, value numeric(18,4) NOT NULL, baseline_value numeric(18,4), source text NOT NULL, measured_at timestamptz NOT NULL, evidence_id uuid, created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.one_on_one_notes (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id), session_id uuid NOT NULL REFERENCES valorapesquisa.one_on_one_sessions(id), author_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id), visibility varchar(20) NOT NULL DEFAULT 'private', topic text NOT NULL, note text NOT NULL, created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.one_on_one_action_links (session_id uuid NOT NULL REFERENCES valorapesquisa.one_on_one_sessions(id), action_item_id uuid NOT NULL REFERENCES valorapesquisa.action_items(id), development_dimension text, linked_by_user_id uuid REFERENCES valorapesquisa.users(id), linked_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY(session_id,action_item_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_targets (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id), name text NOT NULL, description text NOT NULL DEFAULT '', target_value numeric(18,4) NOT NULL, unit varchar(30) NOT NULL, direction varchar(20) NOT NULL DEFAULT 'higher_is_better', source text NOT NULL, periodicity varchar(30) NOT NULL, calculation_type varchar(20) NOT NULL DEFAULT 'manual', owner_user_id uuid REFERENCES valorapesquisa.users(id), active boolean NOT NULL DEFAULT true, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_measurements (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id), indicator_target_id uuid NOT NULL REFERENCES valorapesquisa.indicator_targets(id), value numeric(18,4) NOT NULL, source text NOT NULL, evidence_id uuid, measured_at timestamptz NOT NULL, measured_by_user_id uuid REFERENCES valorapesquisa.users(id), created_at timestamptz NOT NULL DEFAULT now(), UNIQUE(indicator_target_id,measured_at));
CREATE TABLE IF NOT EXISTS valorapesquisa.executive_report_templates (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid REFERENCES valorapesquisa.organizations(id), name text NOT NULL, report_type varchar(30) NOT NULL, description text NOT NULL DEFAULT '', active boolean NOT NULL DEFAULT true, created_by_user_id uuid REFERENCES valorapesquisa.users(id), created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.executive_report_sections (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), template_id uuid NOT NULL REFERENCES valorapesquisa.executive_report_templates(id), section_key varchar(60) NOT NULL, title text NOT NULL, display_order integer NOT NULL, evidence_required boolean NOT NULL DEFAULT true, configuration_json jsonb NOT NULL DEFAULT '{}'::jsonb, UNIQUE(template_id,section_key));
CREATE TABLE IF NOT EXISTS valorapesquisa.organizational_risk_register (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id), title text NOT NULL, description text NOT NULL, severity varchar(20) NOT NULL, probability varchar(20) NOT NULL, status varchar(24) NOT NULL DEFAULT 'open', owner_user_id uuid REFERENCES valorapesquisa.users(id), evidence_summary text NOT NULL, source_type varchar(40) NOT NULL, source_id uuid, review_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.organizational_learning_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id), evolution_cycle_id uuid REFERENCES valorapesquisa.evolution_cycles(id), event_type varchar(40) NOT NULL, learning text NOT NULL, evidence_summary text NOT NULL, source_type varchar(40) NOT NULL, source_id uuid, occurred_at timestamptz NOT NULL DEFAULT now(), created_by_user_id uuid REFERENCES valorapesquisa.users(id), created_at timestamptz NOT NULL DEFAULT now());
ALTER TABLE valorapesquisa.governance_meetings ADD COLUMN IF NOT EXISTS governance_cycle_id uuid REFERENCES valorapesquisa.governance_cycles(id);
ALTER TABLE valorapesquisa.action_item_evidence_links ADD COLUMN IF NOT EXISTS evidence_type varchar(50) NOT NULL DEFAULT 'organizational';
ALTER TABLE valorapesquisa.action_item_evidence_links ADD COLUMN IF NOT EXISTS purpose varchar(30) NOT NULL DEFAULT 'support';
ALTER TABLE valorapesquisa.action_item_evidence_links ADD COLUMN IF NOT EXISTS linked_by_user_id uuid REFERENCES valorapesquisa.users(id);
ALTER TABLE valorapesquisa.action_item_evidence_links ADD COLUMN IF NOT EXISTS linked_at timestamptz NOT NULL DEFAULT now();
ALTER TABLE valorapesquisa.evolution_milestones ADD COLUMN IF NOT EXISTS target_at timestamptz;
ALTER TABLE valorapesquisa.evolution_milestones ADD COLUMN IF NOT EXISTS reached_at timestamptz;
CREATE INDEX IF NOT EXISTS ix_governance_cycles_org_status_period ON valorapesquisa.governance_cycles(organization_id,status,period_end);
CREATE INDEX IF NOT EXISTS ix_governance_meetings_org_schedule ON valorapesquisa.governance_meetings(organization_id,status,scheduled_at);
CREATE INDEX IF NOT EXISTS ix_governance_decisions_org_status ON valorapesquisa.governance_decisions(organization_id,status,priority);
CREATE INDEX IF NOT EXISTS ix_action_item_updates_action_time ON valorapesquisa.action_item_updates(action_item_id,created_at DESC);
CREATE INDEX IF NOT EXISTS ix_evolution_metric_cycle_time ON valorapesquisa.evolution_metric_snapshots(evolution_cycle_id,measured_at DESC);
...
CREATE TABLE IF NOT EXISTS valorapesquisa.workspace_items (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL, item_type varchar(80) NOT NULL,
 title varchar(180) NOT NULL, summary text, status varchar(40) NOT NULL DEFAULT 'pending', priority varchar(20) NOT NULL DEFAULT 'medium',
 due_at timestamptz, owner_user_id uuid, source_type varchar(80), source_id uuid, route varchar(500), metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
 created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), completed_at timestamptz,
 CONSTRAINT workspace_items_priority_ck CHECK(priority IN ('critical','high','medium','low')));
CREATE INDEX IF NOT EXISTS ix_workspace_items_my_day ON valorapesquisa.workspace_items(organization_id,status,priority,due_at);
CREATE INDEX IF NOT EXISTS ix_workspace_items_owner ON valorapesquisa.workspace_items(organization_id,owner_user_id,status);
CREATE TABLE IF NOT EXISTS valorapesquisa.workspace_item_sources (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,workspace_item_id uuid NOT NULL REFERENCES valorapesquisa.workspace_items(id) ON DELETE CASCADE,source_type varchar(80) NOT NULL,source_id uuid NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(organization_id,workspace_item_id,source_type,source_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.workspace_user_preferences (organization_id uuid NOT NULL,user_id uuid NOT NULL,preferences jsonb NOT NULL DEFAULT '{}'::jsonb,updated_at timestamptz NOT NULL DEFAULT now(),PRIMARY KEY(organization_id,user_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.workspace_saved_filters (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,user_id uuid NOT NULL,name varchar(100) NOT NULL,criteria jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(organization_id,user_id,name));
CREATE TABLE IF NOT EXISTS valorapesquisa.global_search_index (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,result_type varchar(80) NOT NULL,title varchar(240) NOT NULL,description text,route varchar(500),source_id uuid,allowed_user_id uuid,search_vector tsvector NOT NULL,updated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(organization_id,result_type,id));
CREATE INDEX IF NOT EXISTS ix_global_search_tenant ON valorapesquisa.global_search_index(organization_id,result_type);
CREATE INDEX IF NOT EXISTS ix_global_search_vector ON valorapesquisa.global_search_index USING gin(search_vector);
CREATE TABLE IF NOT EXISTS valorapesquisa.global_search_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,user_id uuid NOT NULL,term varchar(300) NOT NULL,result_count integer NOT NULL DEFAULT 0,opened_result_id uuid,searched_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.quick_actions (code varchar(80) PRIMARY KEY,label varchar(120) NOT NULL,description varchar(300) NOT NULL,route varchar(500) NOT NULL,icon varchar(60) NOT NULL DEFAULT 'sparkles',sort_order integer NOT NULL DEFAULT 0,enabled boolean NOT NULL DEFAULT true);
CREATE TABLE IF NOT EXISTS valorapesquisa.quick_action_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,user_id uuid NOT NULL,action_code varchar(80) NOT NULL REFERENCES valorapesquisa.quick_actions(code),executed_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.user_recent_items (organization_id uuid NOT NULL,user_id uuid NOT NULL,workspace_item_id uuid NOT NULL REFERENCES valorapesquisa.workspace_items(id) ON DELETE CASCADE,opened_at timestamptz NOT NULL DEFAULT now(),PRIMARY KEY(organization_id,user_id,workspace_item_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.user_pinned_items (organization_id uuid NOT NULL,user_id uuid NOT NULL,workspace_item_id uuid NOT NULL REFERENCES valorapesquisa.workspace_items(id) ON DELETE CASCADE,created_at timestamptz NOT NULL DEFAULT now(),PRIMARY KEY(organization_id,user_id,workspace_item_id));
...
-- Uma atividade pode atender várias prioridades; o vínculo é explícito, auditável e idempotente.
CREATE TABLE IF NOT EXISTS valorapesquisa.priority_action_links(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),priority_id uuid NOT NULL REFERENCES valorapesquisa.executive_priorities(id),action_item_id uuid NOT NULL REFERENCES valorapesquisa.action_items(id),created_by_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),created_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
...
-- Valora Knowledge & Learning Center (idempotent convergence layer).
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_categories (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid,name text NOT NULL,code text NOT NULL,description text,status text NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),UNIQUE NULLS NOT DISTINCT(organization_id,code));
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_articles (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,category_id uuid NOT NULL REFERENCES valorapesquisa.knowledge_categories(id),title text NOT NULL,status text NOT NULL DEFAULT 'draft' CHECK(status IN('draft','review','published','archived')),content text NOT NULL,responsible_user_id uuid NOT NULL,ai_assisted boolean NOT NULL DEFAULT false,current_version integer NOT NULL DEFAULT 1,created_by_user_id uuid NOT NULL,published_at timestamptz,archived_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),search_vector tsvector GENERATED ALWAYS AS (to_tsvector('portuguese',coalesce(title,'')||' '||coalesce(content,''))) STORED);
CREATE INDEX IF NOT EXISTS ix_knowledge_articles_org_status ON valorapesquisa.knowledge_articles(organization_id,status,updated_at DESC);CREATE INDEX IF NOT EXISTS ix_knowledge_articles_search ON valorapesquisa.knowledge_articles USING gin(search_vector);
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_article_versions(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,article_id uuid NOT NULL REFERENCES valorapesquisa.knowledge_articles(id),version integer NOT NULL,title text NOT NULL,content text NOT NULL,created_by_user_id uuid NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(article_id,version));
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_article_tags(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,article_id uuid NOT NULL REFERENCES valorapesquisa.knowledge_articles(id),tag text NOT NULL,UNIQUE(article_id,tag));
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_article_links(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,article_id uuid NOT NULL REFERENCES valorapesquisa.knowledge_articles(id),target_type text NOT NULL,target_id uuid NOT NULL,relationship text NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(article_id,target_type,target_id,relationship));
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_reviews(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,article_id uuid NOT NULL REFERENCES valorapesquisa.knowledge_articles(id),article_version integer NOT NULL,reviewer_user_id uuid NOT NULL,status text NOT NULL CHECK(status IN('pending','approved','changes_requested')),comments text,reviewed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.organizational_lessons(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,title text NOT NULL,learning text NOT NULL,origin_type text NOT NULL CHECK(origin_type IN('action','decision','diagnostic','meeting','report','incident')),origin_reference text NOT NULL,impact text NOT NULL,evidence text,is_critical boolean NOT NULL DEFAULT false,status text NOT NULL DEFAULT 'proposed',created_by_user_id uuid NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),CHECK(NOT is_critical OR nullif(trim(evidence),'') IS NOT NULL));
CREATE TABLE IF NOT EXISTS valorapesquisa.organizational_lesson_links(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,lesson_id uuid NOT NULL REFERENCES valorapesquisa.organizational_lessons(id),target_type text NOT NULL,target_id uuid NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(lesson_id,target_type,target_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.playbooks(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,title text NOT NULL,objective text NOT NULL,area text NOT NULL,status text NOT NULL DEFAULT 'draft' CHECK(status IN('draft','review','published','archived')),created_by_user_id uuid NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.playbook_steps(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,playbook_id uuid NOT NULL REFERENCES valorapesquisa.playbooks(id),step_order integer NOT NULL CHECK(step_order>0),description text NOT NULL,suggested_owner text NOT NULL,evidence_guidance text,UNIQUE(playbook_id,step_order));
CREATE TABLE IF NOT EXISTS valorapesquisa.learning_paths(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,title text NOT NULL,objective text NOT NULL,status text NOT NULL DEFAULT 'draft',created_by_user_id uuid NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.learning_path_items(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,learning_path_id uuid NOT NULL REFERENCES valorapesquisa.learning_paths(id),item_order integer NOT NULL,item_type text NOT NULL,article_id uuid REFERENCES valorapesquisa.knowledge_articles(id),playbook_id uuid REFERENCES valorapesquisa.playbooks(id),UNIQUE(learning_path_id,item_order));
CREATE TABLE IF NOT EXISTS valorapesquisa.learning_path_assignments(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,learning_path_id uuid NOT NULL REFERENCES valorapesquisa.learning_paths(id),user_id uuid,profile_code text,assigned_by_user_id uuid NOT NULL,assigned_at timestamptz NOT NULL DEFAULT now(),due_at timestamptz,CHECK(user_id IS NOT NULL OR nullif(trim(profile_code),'') IS NOT NULL));
CREATE TABLE IF NOT EXISTS valorapesquisa.learning_progress(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,assignment_id uuid NOT NULL REFERENCES valorapesquisa.learning_path_assignments(id),item_id uuid REFERENCES valorapesquisa.learning_path_items(id),progress_percent integer NOT NULL DEFAULT 0 CHECK(progress_percent BETWEEN 0 AND 100),completed_at timestamptz,updated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(assignment_id,item_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.cognitive_dictionary_terms(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,code text NOT NULL,name text NOT NULL,definition text NOT NULL CHECK(nullif(trim(definition),'') IS NOT NULL),category text NOT NULL,methodological_use text NOT NULL,status text NOT NULL DEFAULT 'draft',traceability_reference text,created_by_user_id uuid NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),UNIQUE(organization_id,code));
CREATE TABLE IF NOT EXISTS valorapesquisa.cognitive_dictionary_relations(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,source_term_id uuid NOT NULL REFERENCES valorapesquisa.cognitive_dictionary_terms(id),target_term_id uuid NOT NULL REFERENCES valorapesquisa.cognitive_dictionary_terms(id),relation_type text NOT NULL,usage_example text,UNIQUE(source_term_id,target_term_id,relation_type));
CREATE TABLE IF NOT EXISTS valorapesquisa.knowledge_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,event_type text NOT NULL,entity_type text NOT NULL,entity_id uuid NOT NULL,actor_user_id uuid,correlation_id text NOT NULL,metadata jsonb NOT NULL DEFAULT '{}',occurred_at timestamptz NOT NULL DEFAULT now());CREATE INDEX IF NOT EXISTS ix_knowledge_events_org_time ON valorapesquisa.knowledge_events(organization_id,occurred_at DESC);
INSERT INTO valorapesquisa.knowledge_categories(organization_id,name,code,description) VALUES(NULL,'Metodologia Valora','metodologia','Conceitos e práticas metodológicas verificadas'),(NULL,'Decisões e Governança','decisoes-governanca','Aprendizados originados em decisões'),(NULL,'Práticas Organizacionais','praticas-organizacionais','Boas práticas baseadas em experiência real') ON CONFLICT(organization_id,code) DO NOTHING;


-- Valora Indicators & Analytics Center (idempotent convergence layer).
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_categories (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,name text NOT NULL,code text NOT NULL,description text,status text NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(organization_id,code));
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_sources (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,name text NOT NULL,source_type text NOT NULL CHECK(source_type IN('manual','calculated','data_hub','diagnostic','integration')),reliability_status text NOT NULL DEFAULT 'declared',data_hub_connection_id uuid,description text,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(organization_id,name));
CREATE TABLE IF NOT EXISTS valorapesquisa.indicators (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,name text NOT NULL,category_id uuid NOT NULL REFERENCES valorapesquisa.indicator_categories(id),unit text NOT NULL,source_id uuid REFERENCES valorapesquisa.indicator_sources(id),responsible_user_id uuid NOT NULL,periodicity text NOT NULL CHECK(periodicity IN('daily','weekly','monthly','quarterly','yearly')),status text NOT NULL DEFAULT 'active' CHECK(status IN('draft','active','incomplete','archived')),is_calculated boolean NOT NULL DEFAULT false,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),archived_at timestamptz);
CREATE INDEX IF NOT EXISTS ix_indicators_org_status ON valorapesquisa.indicators(organization_id,status);
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_formulas (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,indicator_id uuid NOT NULL REFERENCES valorapesquisa.indicators(id),expression text NOT NULL,description text,is_active boolean NOT NULL DEFAULT true,created_at timestamptz NOT NULL DEFAULT now());

-- Earlier installations used indicator_targets/measurements for a legacy module. Converge without dropping its data.
ALTER TABLE valorapesquisa.indicator_targets ADD COLUMN IF NOT EXISTS indicator_id uuid REFERENCES valorapesquisa.indicators(id);
ALTER TABLE valorapesquisa.indicator_targets ADD COLUMN IF NOT EXISTS period_start timestamptz;
ALTER TABLE valorapesquisa.indicator_targets ADD COLUMN IF NOT EXISTS period_end timestamptz;
ALTER TABLE valorapesquisa.indicator_targets ADD COLUMN IF NOT EXISTS comparison_rule text;
ALTER TABLE valorapesquisa.indicator_targets ADD COLUMN IF NOT EXISTS responsible_user_id uuid;
ALTER TABLE valorapesquisa.indicator_targets ADD COLUMN IF NOT EXISTS status text DEFAULT 'active';
ALTER TABLE valorapesquisa.indicator_targets ALTER COLUMN name DROP NOT NULL; ALTER TABLE valorapesquisa.indicator_targets ALTER COLUMN unit DROP NOT NULL; ALTER TABLE valorapesquisa.indicator_targets ALTER COLUMN source DROP NOT NULL; ALTER TABLE valorapesquisa.indicator_targets ALTER COLUMN periodicity DROP NOT NULL;
CREATE INDEX IF NOT EXISTS ix_indicator_targets_indicator_period ON valorapesquisa.indicator_targets(organization_id,indicator_id,period_start,period_end);
ALTER TABLE valorapesquisa.indicator_measurements ADD COLUMN IF NOT EXISTS indicator_id uuid REFERENCES valorapesquisa.indicators(id);
ALTER TABLE valorapesquisa.indicator_measurements ADD COLUMN IF NOT EXISTS source_name text;
ALTER TABLE valorapesquisa.indicator_measurements ADD COLUMN IF NOT EXISTS responsible_user_id uuid;
ALTER TABLE valorapesquisa.indicator_measurements ADD COLUMN IF NOT EXISTS justification text;
ALTER TABLE valorapesquisa.indicator_measurements ADD COLUMN IF NOT EXISTS revision integer NOT NULL DEFAULT 1;
ALTER TABLE valorapesquisa.indicator_measurements ADD COLUMN IF NOT EXISTS revised_measurement_id uuid;
ALTER TABLE valorapesquisa.indicator_measurements ADD COLUMN IF NOT EXISTS data_hub_import_id uuid;
ALTER TABLE valorapesquisa.indicator_measurements ALTER COLUMN indicator_target_id DROP NOT NULL; ALTER TABLE valorapesquisa.indicator_measurements ALTER COLUMN source DROP NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS ux_indicator_measurement_period_revision ON valorapesquisa.indicator_measurements(organization_id,indicator_id,measured_at,revision) WHERE indicator_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_indicator_measurements_timeline ON valorapesquisa.indicator_measurements(organization_id,indicator_id,measured_at DESC);
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_alert_rules (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,indicator_id uuid NOT NULL REFERENCES valorapesquisa.indicators(id),name text NOT NULL,severity text NOT NULL CHECK(severity IN('low','medium','high','critical')),condition_type text NOT NULL,threshold_value numeric,active boolean NOT NULL DEFAULT true,created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_alerts (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,indicator_id uuid NOT NULL REFERENCES valorapesquisa.indicators(id),rule_id uuid NOT NULL REFERENCES valorapesquisa.indicator_alert_rules(id),severity text NOT NULL,message text NOT NULL,status text NOT NULL DEFAULT 'open' CHECK(status IN('open','acknowledged','resolved')),created_at timestamptz NOT NULL DEFAULT now(),resolved_at timestamptz,resolved_by_user_id uuid);
CREATE INDEX IF NOT EXISTS ix_indicator_alerts_dashboard ON valorapesquisa.indicator_alerts(organization_id,status,severity,created_at DESC);
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_action_links (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,indicator_id uuid NOT NULL REFERENCES valorapesquisa.indicators(id),action_id uuid NOT NULL,alert_id uuid REFERENCES valorapesquisa.indicator_alerts(id),influence_note text NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(organization_id,indicator_id,action_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_diagnostic_links (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,indicator_id uuid NOT NULL REFERENCES valorapesquisa.indicators(id),diagnostic_id uuid NOT NULL,evidence_strength text NOT NULL DEFAULT 'contextual',created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(organization_id,indicator_id,diagnostic_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.executive_scorecards (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,name text NOT NULL,objective text NOT NULL,period_start timestamptz NOT NULL,period_end timestamptz NOT NULL,status text NOT NULL DEFAULT 'active',created_by_user_id uuid NOT NULL,created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.executive_scorecard_items (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,scorecard_id uuid NOT NULL REFERENCES valorapesquisa.executive_scorecards(id),indicator_id uuid NOT NULL REFERENCES valorapesquisa.indicators(id),display_order integer NOT NULL,target_id uuid REFERENCES valorapesquisa.indicator_targets(id),UNIQUE(scorecard_id,indicator_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.analytics_snapshots (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,name text NOT NULL,limitation text NOT NULL,created_by_user_id uuid NOT NULL,captured_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.analytics_snapshot_items (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,snapshot_id uuid NOT NULL REFERENCES valorapesquisa.analytics_snapshots(id),indicator_id uuid NOT NULL REFERENCES valorapesquisa.indicators(id),value numeric,source_name text,measurement_count integer NOT NULL DEFAULT 0,trend text NOT NULL DEFAULT 'insufficient_data',UNIQUE(snapshot_id,indicator_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.indicator_trend_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL,indicator_id uuid NOT NULL REFERENCES valorapesquisa.indicators(id),previous_trend text,current_trend text NOT NULL,sample_size integer NOT NULL,limitation text NOT NULL,occurred_at timestamptz NOT NULL DEFAULT now());
...
CREATE TABLE IF NOT EXISTS valorapesquisa.advisor_messages (
 ...
 created_at timestamptz NOT NULL DEFAULT now());
ALTER TABLE valorapesquisa.advisor_messages ADD COLUMN IF NOT EXISTS organization_id uuid;
ALTER TABLE valorapesquisa.advisor_messages ADD COLUMN IF NOT EXISTS conversation_id uuid;
ALTER TABLE valorapesquisa.advisor_messages ADD COLUMN IF NOT EXISTS user_id uuid;
ALTER TABLE valorapesquisa.advisor_messages ADD COLUMN IF NOT EXISTS role varchar(20);
ALTER TABLE valorapesquisa.advisor_messages ADD COLUMN IF NOT EXISTS content text;
ALTER TABLE valorapesquisa.advisor_messages ADD COLUMN IF NOT EXISTS confidence varchar(24) DEFAULT 'low';
ALTER TABLE valorapesquisa.advisor_messages ADD COLUMN IF NOT EXISTS limitations text[] DEFAULT ARRAY[]::text[];
ALTER TABLE valorapesquisa.advisor_messages ADD COLUMN IF NOT EXISTS created_at timestamptz DEFAULT now();
...
ALTER TABLE valorapesquisa.advisor_context_bundles ADD COLUMN IF NOT EXISTS organization_id uuid;
ALTER TABLE valorapesquisa.advisor_context_bundles ADD COLUMN IF NOT EXISTS conversation_id uuid;
ALTER TABLE valorapesquisa.advisor_context_bundles ADD COLUMN IF NOT EXISTS message_id uuid;
ALTER TABLE valorapesquisa.advisor_context_bundles ADD COLUMN IF NOT EXISTS purpose text;
ALTER TABLE valorapesquisa.advisor_context_bundles ADD COLUMN IF NOT EXISTS created_at timestamptz DEFAULT now();
...
ALTER TABLE valorapesquisa.advisor_context_sources ADD COLUMN IF NOT EXISTS organization_id uuid;
ALTER TABLE valorapesquisa.advisor_context_sources ADD COLUMN IF NOT EXISTS bundle_id uuid;
ALTER TABLE valorapesquisa.advisor_context_sources ADD COLUMN IF NOT EXISTS source_type varchar(40);
ALTER TABLE valorapesquisa.advisor_context_sources ADD COLUMN IF NOT EXISTS source_id uuid;
ALTER TABLE valorapesquisa.advisor_context_sources ADD COLUMN IF NOT EXISTS title text;
ALTER TABLE valorapesquisa.advisor_context_sources ADD COLUMN IF NOT EXISTS snapshot jsonb DEFAULT '{}';
ALTER TABLE valorapesquisa.advisor_context_sources ADD COLUMN IF NOT EXISTS collected_at timestamptz DEFAULT now();
...
ALTER TABLE valorapesquisa.advisor_evidence_citations ADD COLUMN IF NOT EXISTS organization_id uuid;
ALTER TABLE valorapesquisa.advisor_evidence_citations ADD COLUMN IF NOT EXISTS message_id uuid;
ALTER TABLE valorapesquisa.advisor_evidence_citations ADD COLUMN IF NOT EXISTS source_type varchar(40);
ALTER TABLE valorapesquisa.advisor_evidence_citations ADD COLUMN IF NOT EXISTS source_id uuid;
ALTER TABLE valorapesquisa.advisor_evidence_citations ADD COLUMN IF NOT EXISTS title text;
ALTER TABLE valorapesquisa.advisor_evidence_citations ADD COLUMN IF NOT EXISTS excerpt text;
ALTER TABLE valorapesquisa.advisor_evidence_citations ADD COLUMN IF NOT EXISTS strength varchar(24) DEFAULT 'contextual';
ALTER TABLE valorapesquisa.advisor_evidence_citations ADD COLUMN IF NOT EXISTS created_at timestamptz DEFAULT now();
...
ALTER TABLE valorapesquisa.advisor_prompt_templates ADD COLUMN IF NOT EXISTS organization_id uuid;
ALTER TABLE valorapesquisa.advisor_prompt_templates ADD COLUMN IF NOT EXISTS code varchar(80);
ALTER TABLE valorapesquisa.advisor_prompt_templates ADD COLUMN IF NOT EXISTS name varchar(160);
ALTER TABLE valorapesquisa.advisor_prompt_templates ADD COLUMN IF NOT EXISTS area varchar(80);
ALTER TABLE valorapesquisa.advisor_prompt_templates ADD COLUMN IF NOT EXISTS status varchar(24) DEFAULT 'draft';
ALTER TABLE valorapesquisa.advisor_prompt_templates ADD COLUMN IF NOT EXISTS created_by_user_id uuid;
ALTER TABLE valorapesquisa.advisor_prompt_templates ADD COLUMN IF NOT EXISTS created_at timestamptz DEFAULT now();
ALTER TABLE valorapesquisa.advisor_prompt_templates ADD COLUMN IF NOT EXISTS updated_at timestamptz DEFAULT now();
...
ALTER TABLE valorapesquisa.advisor_guardrail_events ADD COLUMN IF NOT EXISTS organization_id uuid;
ALTER TABLE valorapesquisa.advisor_guardrail_events ADD COLUMN IF NOT EXISTS user_id uuid;
ALTER TABLE valorapesquisa.advisor_guardrail_events ADD COLUMN IF NOT EXISTS conversation_id uuid;
ALTER TABLE valorapesquisa.advisor_guardrail_events ADD COLUMN IF NOT EXISTS rule_code varchar(80);
ALTER TABLE valorapesquisa.advisor_guardrail_events ADD COLUMN IF NOT EXISTS reason text;
ALTER TABLE valorapesquisa.advisor_guardrail_events ADD COLUMN IF NOT EXISTS context jsonb DEFAULT '{}';
ALTER TABLE valorapesquisa.advisor_guardrail_events ADD COLUMN IF NOT EXISTS created_at timestamptz DEFAULT now();
...
ALTER TABLE valorapesquisa.advisor_action_suggestions ADD COLUMN IF NOT EXISTS organization_id uuid;
ALTER TABLE valorapesquisa.advisor_action_suggestions ADD COLUMN IF NOT EXISTS status varchar(24) DEFAULT 'pending';
ALTER TABLE valorapesquisa.advisor_action_suggestions ADD COLUMN IF NOT EXISTS created_at timestamptz DEFAULT now();
...
ALTER TABLE valorapesquisa.advisor_usage_events ADD COLUMN IF NOT EXISTS organization_id uuid;
ALTER TABLE valorapesquisa.advisor_usage_events ADD COLUMN IF NOT EXISTS event_name varchar(100);
ALTER TABLE valorapesquisa.advisor_usage_events ADD COLUMN IF NOT EXISTS created_at timestamptz DEFAULT now();
...
-- Valora Process Intelligence & Automation Studio™
-- Estrutura aditiva e idempotente: preserva instalações limpas ou parcialmente atualizadas.
CREATE TABLE IF NOT EXISTS valorapesquisa.process_definitions (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),
 name varchar(160) NOT NULL, description text NOT NULL DEFAULT '', category varchar(80) NOT NULL,
 owner_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id), status varchar(24) NOT NULL DEFAULT 'draft' CHECK(status IN('draft','published','archived')),
 version_number integer NOT NULL DEFAULT 1 CHECK(version_number>0), parent_definition_id uuid REFERENCES valorapesquisa.process_definitions(id),
 metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb, published_at timestamptz, created_by_user_id uuid REFERENCES valorapesquisa.users(id),
 created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), deleted_at timestamptz
);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_steps (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id), process_definition_id uuid NOT NULL REFERENCES valorapesquisa.process_definitions(id),
 name varchar(160) NOT NULL, description text, step_type varchar(24) NOT NULL CHECK(step_type IN('initial','task','approval','automation','final')), order_index integer NOT NULL CHECK(order_index>=0),
 responsible_role_id uuid, sla_hours integer CHECK(sla_hours>0), requires_approval boolean NOT NULL DEFAULT false, evidence_required boolean NOT NULL DEFAULT false,
 metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), deleted_at timestamptz,
 UNIQUE(process_definition_id,order_index)
);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_step_transitions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_definition_id uuid NOT NULL REFERENCES valorapesquisa.process_definitions(id),from_step_id uuid NOT NULL REFERENCES valorapesquisa.process_steps(id),to_step_id uuid NOT NULL REFERENCES valorapesquisa.process_steps(id),condition_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(from_step_id,to_step_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.process_instances (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_definition_id uuid NOT NULL REFERENCES valorapesquisa.process_definitions(id),source_type varchar(24) NOT NULL,source_id uuid,title varchar(180) NOT NULL,status varchar(24) NOT NULL DEFAULT 'running' CHECK(status IN('running','paused','completed','cancelled')),priority varchar(16) NOT NULL DEFAULT 'medium',current_step_id uuid REFERENCES valorapesquisa.process_steps(id),responsible_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),started_at timestamptz NOT NULL DEFAULT now(),completed_at timestamptz,due_at timestamptz,created_by_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz
);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_instance_steps (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_instance_id uuid NOT NULL REFERENCES valorapesquisa.process_instances(id),step_id uuid NOT NULL REFERENCES valorapesquisa.process_steps(id),status varchar(24) NOT NULL DEFAULT 'pending',started_at timestamptz,completed_at timestamptz,due_at timestamptz,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_assignments (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_instance_id uuid NOT NULL REFERENCES valorapesquisa.process_instances(id),step_id uuid REFERENCES valorapesquisa.process_steps(id),assigned_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),assigned_by_user_id uuid REFERENCES valorapesquisa.users(id),status varchar(24) NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_approvals (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_instance_id uuid NOT NULL REFERENCES valorapesquisa.process_instances(id),step_id uuid NOT NULL REFERENCES valorapesquisa.process_steps(id),approver_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),status varchar(24) NOT NULL DEFAULT 'pending' CHECK(status IN('pending','approved','rejected','returned')),decision_reason text,return_step_id uuid REFERENCES valorapesquisa.process_steps(id),decided_at timestamptz,decided_by_user_id uuid REFERENCES valorapesquisa.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_sla_rules (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_definition_id uuid NOT NULL REFERENCES valorapesquisa.process_definitions(id),step_id uuid REFERENCES valorapesquisa.process_steps(id),sla_hours integer NOT NULL CHECK(sla_hours>0),at_risk_percentage integer NOT NULL DEFAULT 80 CHECK(at_risk_percentage BETWEEN 1 AND 100),status varchar(24) NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_sla_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_instance_id uuid NOT NULL REFERENCES valorapesquisa.process_instances(id),step_id uuid REFERENCES valorapesquisa.process_steps(id),event_type varchar(32) NOT NULL,due_at timestamptz NOT NULL,occurred_at timestamptz NOT NULL DEFAULT now(),metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.process_automation_rules (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_definition_id uuid NOT NULL REFERENCES valorapesquisa.process_definitions(id),name varchar(160) NOT NULL,trigger_type varchar(40) NOT NULL,condition_json jsonb NOT NULL DEFAULT '{}'::jsonb,action_json jsonb NOT NULL DEFAULT '{}'::jsonb,status varchar(24) NOT NULL DEFAULT 'draft',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_automation_executions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),automation_rule_id uuid NOT NULL REFERENCES valorapesquisa.process_automation_rules(id),process_instance_id uuid REFERENCES valorapesquisa.process_instances(id),status varchar(24) NOT NULL,result_json jsonb NOT NULL DEFAULT '{}'::jsonb,technical_message text,started_at timestamptz NOT NULL DEFAULT now(),completed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.process_evidence_links (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_instance_id uuid NOT NULL REFERENCES valorapesquisa.process_instances(id),step_id uuid REFERENCES valorapesquisa.process_steps(id),evidence_type varchar(40) NOT NULL,evidence_id uuid NOT NULL,evidence_summary text NOT NULL,created_by_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),created_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_bottleneck_insights (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),process_definition_id uuid NOT NULL REFERENCES valorapesquisa.process_definitions(id),step_id uuid REFERENCES valorapesquisa.process_steps(id),insight_type varchar(40) NOT NULL,evidence_summary text NOT NULL,average_duration_hours numeric(12,2),occurrence_count integer NOT NULL DEFAULT 0,status varchar(24) NOT NULL DEFAULT 'open',detected_at timestamptz NOT NULL DEFAULT now(),metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_templates (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid REFERENCES valorapesquisa.organizations(id),name varchar(160) NOT NULL,description text NOT NULL DEFAULT '',category varchar(80) NOT NULL,status varchar(24) NOT NULL DEFAULT 'active',definition_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.process_audit_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),entity_type varchar(40) NOT NULL,entity_id uuid NOT NULL,event_type varchar(80) NOT NULL,performed_by_user_id uuid REFERENCES valorapesquisa.users(id),summary text NOT NULL,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_process_definitions_org_status ON valorapesquisa.process_definitions(organization_id,status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_process_instances_org_status_due ON valorapesquisa.process_instances(organization_id,status,due_at) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_process_approvals_approver_status ON valorapesquisa.process_approvals(approver_user_id,status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_process_audit_entity ON valorapesquisa.process_audit_events(organization_id,entity_id,created_at DESC);
INSERT INTO valorapesquisa.permissions(code,name,description,module_code) VALUES
 ('processes.view','Consultar processos','Consultar o painel executivo de processos.','organizational_intelligence'),
 ('processes.manage','Gerenciar processos','Gerenciar processos da organização.','organizational_intelligence'),
 ('processes.definitions.create','Criar definições de processo','Criar processos em rascunho.','organizational_intelligence'),
 ('processes.definitions.publish','Publicar definições de processo','Publicar versões estruturalmente válidas.','organizational_intelligence'),
 ('processes.builder.manage','Gerenciar Process Builder','Configurar etapas, transições e responsabilidades.','organizational_intelligence'),
 ('processes.instances.view','Consultar instâncias','Consultar execuções reais de processos.','organizational_intelligence'),
 ('processes.instances.manage','Gerenciar instâncias','Iniciar e avançar execuções de processos.','organizational_intelligence'),
 ('processes.approvals.manage','Gerenciar aprovações','Registrar decisões humanas em fluxos.','organizational_intelligence'),
 ('processes.sla.view','Consultar SLA de processos','Consultar prazos, riscos e atrasos.','organizational_intelligence'),
 ('processes.insights.view','Consultar gargalos','Consultar gargalos baseados no histórico.','organizational_intelligence'),
 ('processes.templates.manage','Gerenciar templates de processos','Aplicar e adaptar modelos organizacionais.','organizational_intelligence'),
 ('processes.automation.manage','Gerenciar automações de processos','Configurar automações explícitas e auditáveis.','organizational_intelligence')
ON CONFLICT(code) DO UPDATE SET name=excluded.name,description=excluded.description,module_code=excluded.module_code,updated_at=now();

-- Valora Benchmark & Comparative Intelligence Center (tenant-safe and idempotent)
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_cohorts(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),name varchar(160) NOT NULL,description text NOT NULL DEFAULT '',segment varchar(100) NOT NULL,industry varchar(100) NOT NULL,company_size_range varchar(80) NOT NULL,region varchar(100) NOT NULL,minimum_sample_size integer NOT NULL DEFAULT 5 CHECK(minimum_sample_size>=5),status varchar(24) NOT NULL DEFAULT 'active',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_cohort_members(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),cohort_id uuid NOT NULL REFERENCES valorapesquisa.benchmark_cohorts(id),member_organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(cohort_id,member_organization_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_snapshots(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),source_type varchar(30) NOT NULL,source_id uuid NOT NULL,period_start date NOT NULL,period_end date NOT NULL,maturity_score numeric(7,2) NOT NULL CHECK(maturity_score BETWEEN 0 AND 100),confidence_level varchar(16) NOT NULL CHECK(confidence_level IN('low','medium','high')),sample_size integer NOT NULL CHECK(sample_size>0),metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,CHECK(period_end>=period_start));
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_metrics(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),snapshot_id uuid NOT NULL REFERENCES valorapesquisa.benchmark_snapshots(id),metric_key varchar(120) NOT NULL,metric_name varchar(160) NOT NULL,dimension varchar(120),numeric_value numeric(18,4) NOT NULL,data_kind varchar(20) NOT NULL DEFAULT 'real',evidence_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(snapshot_id,metric_key,dimension));
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_dimension_scores(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),snapshot_id uuid NOT NULL REFERENCES valorapesquisa.benchmark_snapshots(id),dimension varchar(120) NOT NULL,score numeric(7,2) NOT NULL CHECK(score BETWEEN 0 AND 100),confidence_level varchar(16) NOT NULL,sample_size integer NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(snapshot_id,dimension));
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_percentiles(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),snapshot_id uuid NOT NULL REFERENCES valorapesquisa.benchmark_snapshots(id),cohort_id uuid REFERENCES valorapesquisa.benchmark_cohorts(id),metric_key varchar(120) NOT NULL,percentile numeric(6,2) NOT NULL CHECK(percentile BETWEEN 0 AND 100),sample_size integer NOT NULL CHECK(sample_size>=5),confidence_level varchar(16) NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_comparison_requests(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),comparison_type varchar(20) NOT NULL CHECK(comparison_type IN('history','units','cohort')),cohort_id uuid REFERENCES valorapesquisa.benchmark_cohorts(id),segment varchar(100) NOT NULL,criterion varchar(120) NOT NULL,period_start date NOT NULL,period_end date NOT NULL,dimension varchar(120),indicator varchar(120),requested_by_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),status varchar(24) NOT NULL DEFAULT 'processing',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_comparison_results(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),comparison_request_id uuid NOT NULL REFERENCES valorapesquisa.benchmark_comparison_requests(id),is_available boolean NOT NULL,confidence_level varchar(20) NOT NULL,sample_size integer NOT NULL DEFAULT 0,limitations_json jsonb NOT NULL DEFAULT '[]'::jsonb,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(comparison_request_id));
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_privacy_rules(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),minimum_sample_size integer NOT NULL DEFAULT 5 CHECK(minimum_sample_size>=5),k_anonymity integer NOT NULL DEFAULT 5 CHECK(k_anonymity>=5),status varchar(24) NOT NULL DEFAULT 'active',suppression_reason text,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_insights(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),comparison_request_id uuid REFERENCES valorapesquisa.benchmark_comparison_requests(id),insight_type varchar(40) NOT NULL,title varchar(180) NOT NULL,description text NOT NULL,evidence_summary text NOT NULL,confidence_level varchar(16) NOT NULL,status varchar(24) NOT NULL DEFAULT 'open',action_id uuid REFERENCES valorapesquisa.action_items(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_exports(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),comparison_request_id uuid NOT NULL REFERENCES valorapesquisa.benchmark_comparison_requests(id),format varchar(12) NOT NULL,status varchar(24) NOT NULL,requested_by_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id),storage_reference text,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
ALTER TABLE valorapesquisa.benchmark_snapshots ADD COLUMN IF NOT EXISTS source_type varchar(30) NOT NULL DEFAULT 'diagnostic', ADD COLUMN IF NOT EXISTS source_id uuid, ADD COLUMN IF NOT EXISTS period_start date NOT NULL DEFAULT CURRENT_DATE, ADD COLUMN IF NOT EXISTS period_end date NOT NULL DEFAULT CURRENT_DATE, ADD COLUMN IF NOT EXISTS confidence_level varchar(16) NOT NULL DEFAULT 'low', ADD COLUMN IF NOT EXISTS sample_size integer NOT NULL DEFAULT 1;
ALTER TABLE valorapesquisa.benchmark_insights ADD COLUMN IF NOT EXISTS comparison_request_id uuid REFERENCES valorapesquisa.benchmark_comparison_requests(id), ADD COLUMN IF NOT EXISTS insight_type varchar(40) NOT NULL DEFAULT 'opportunity', ADD COLUMN IF NOT EXISTS title varchar(180) NOT NULL DEFAULT 'Insight comparativo', ADD COLUMN IF NOT EXISTS description text NOT NULL DEFAULT '', ADD COLUMN IF NOT EXISTS evidence_summary text NOT NULL DEFAULT 'Evidência não consolidada.', ADD COLUMN IF NOT EXISTS confidence_level varchar(16) NOT NULL DEFAULT 'low', ADD COLUMN IF NOT EXISTS status varchar(24) NOT NULL DEFAULT 'open', ADD COLUMN IF NOT EXISTS action_id uuid REFERENCES valorapesquisa.action_items(id);
ALTER TABLE valorapesquisa.benchmark_exports ADD COLUMN IF NOT EXISTS comparison_request_id uuid REFERENCES valorapesquisa.benchmark_comparison_requests(id), ADD COLUMN IF NOT EXISTS requested_by_user_id uuid REFERENCES valorapesquisa.users(id);
CREATE TABLE IF NOT EXISTS valorapesquisa.benchmark_audit_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),entity_type varchar(40) NOT NULL,entity_id uuid NOT NULL,event_type varchar(100) NOT NULL,performed_by_user_id uuid REFERENCES valorapesquisa.users(id),summary text NOT NULL,metadata_json jsonb NOT NULL DEFAULT '{}'::jsonb,created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_benchmark_cohorts_org ON valorapesquisa.benchmark_cohorts(organization_id,status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_benchmark_snapshots_org_period ON valorapesquisa.benchmark_snapshots(organization_id,period_end DESC) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_benchmark_comparisons_org ON valorapesquisa.benchmark_comparison_requests(organization_id,created_at DESC) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_benchmark_audit_org ON valorapesquisa.benchmark_audit_events(organization_id,created_at DESC);
INSERT INTO valorapesquisa.permissions(code,name,description,module_code) VALUES
('benchmarks.view','Consultar benchmarks','Consultar comparações anônimas e histórico.','organizational_intelligence'),
('benchmarks.manage','Gerenciar benchmarks','Gerenciar o centro comparativo.','organizational_intelligence'),
('benchmarks.cohorts.manage','Gerenciar coortes','Configurar coortes e critérios anônimos.','organizational_intelligence'),
('benchmarks.compare','Comparar benchmarks','Solicitar comparações seguras.','organizational_intelligence'),
('benchmarks.insights.view','Consultar insights comparativos','Consultar insights sustentados por evidência.','organizational_intelligence'),
('benchmarks.exports.create','Exportar benchmarks','Gerar exportações autorizadas.','organizational_intelligence'),
('benchmarks.privacy.manage','Gerenciar privacidade de benchmarks','Configurar k-anonymity e supressão.','organizational_intelligence')
ON CONFLICT(code) DO UPDATE SET name=excluded.name,description=excluded.description,module_code=excluded.module_code,updated_at=now();

-- Valora People, Culture & Leadership Intelligence Center (aditivo e idempotente)
CREATE TABLE IF NOT EXISTS valorapesquisa.people_profiles(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),display_name varchar(120) NOT NULL,role_title varchar(120),status varchar(20) NOT NULL DEFAULT 'active' CHECK(status IN('active','inactive','anonymized')),created_by_user_id uuid REFERENCES valorapesquisa.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.people_teams(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),name varchar(120) NOT NULL,responsible_profile_id uuid NOT NULL REFERENCES valorapesquisa.people_profiles(id),created_by_user_id uuid REFERENCES valorapesquisa.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.people_team_members(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),team_id uuid NOT NULL REFERENCES valorapesquisa.people_teams(id),profile_id uuid NOT NULL REFERENCES valorapesquisa.people_profiles(id),leadership_type varchar(20) CHECK(leadership_type IN('formal','functional')),started_at timestamptz NOT NULL DEFAULT now(),ended_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(team_id,profile_id,started_at));
CREATE TABLE IF NOT EXISTS valorapesquisa.people_leadership_roles(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),profile_id uuid NOT NULL REFERENCES valorapesquisa.people_profiles(id),team_id uuid REFERENCES valorapesquisa.people_teams(id),role_type varchar(20) NOT NULL CHECK(role_type IN('formal','functional')),title varchar(120) NOT NULL,started_at timestamptz NOT NULL DEFAULT now(),ended_at timestamptz,created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.people_culture_dimensions(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),name varchar(120) NOT NULL,description text NOT NULL,minimum_sample_size integer NOT NULL DEFAULT 5 CHECK(minimum_sample_size>=5),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(organization_id,name));
CREATE TABLE IF NOT EXISTS valorapesquisa.people_culture_assessments(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),dimension_id uuid NOT NULL REFERENCES valorapesquisa.people_culture_dimensions(id),source varchar(80) NOT NULL,evidence_summary text NOT NULL,score numeric(5,2) NOT NULL CHECK(score BETWEEN 0 AND 100),sample_size integer NOT NULL CHECK(sample_size>0),confidence varchar(16) NOT NULL CHECK(confidence IN('low','medium','high')),limitations text NOT NULL,assessed_at timestamptz NOT NULL DEFAULT now(),created_by_user_id uuid REFERENCES valorapesquisa.users(id),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.people_engagement_signals(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),source varchar(80) NOT NULL,evidence_summary text NOT NULL,score numeric(5,2) CHECK(score BETWEEN 0 AND 100),confidence varchar(16) NOT NULL DEFAULT 'low',limitations text NOT NULL,observed_at timestamptz NOT NULL DEFAULT now(),created_by_user_id uuid REFERENCES valorapesquisa.users(id),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.people_competency_frameworks(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),name varchar(120) NOT NULL,description text NOT NULL,scale_max numeric(4,2) NOT NULL DEFAULT 5,version integer NOT NULL DEFAULT 1,status varchar(20) NOT NULL DEFAULT 'draft',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.people_competencies(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),framework_id uuid NOT NULL REFERENCES valorapesquisa.people_competency_frameworks(id),name varchar(120) NOT NULL,description text NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(framework_id,name));
CREATE TABLE IF NOT EXISTS valorapesquisa.people_competency_assessments(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),competency_id uuid NOT NULL REFERENCES valorapesquisa.people_competencies(id),subject_type varchar(12) NOT NULL CHECK(subject_type IN('person','team')),profile_id uuid REFERENCES valorapesquisa.people_profiles(id),team_id uuid REFERENCES valorapesquisa.people_teams(id),expected_level numeric(4,2) NOT NULL,observed_level numeric(4,2) NOT NULL,evidence_summary text NOT NULL,human_reviewed boolean NOT NULL DEFAULT false,reviewed_by_user_id uuid REFERENCES valorapesquisa.users(id),assessed_at timestamptz NOT NULL DEFAULT now(),created_by_user_id uuid REFERENCES valorapesquisa.users(id),deleted_at timestamptz,CHECK((profile_id IS NOT NULL)::int+(team_id IS NOT NULL)::int=1));
CREATE TABLE IF NOT EXISTS valorapesquisa.people_development_plans(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),subject_type varchar(12) NOT NULL CHECK(subject_type IN('person','team')),profile_id uuid REFERENCES valorapesquisa.people_profiles(id),team_id uuid REFERENCES valorapesquisa.people_teams(id),objective varchar(500) NOT NULL,responsible_profile_id uuid NOT NULL REFERENCES valorapesquisa.people_profiles(id),due_date date NOT NULL,status varchar(20) NOT NULL DEFAULT 'active',action_plan_id uuid REFERENCES valorapesquisa.action_plans(id),created_by_user_id uuid REFERENCES valorapesquisa.users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,CHECK((profile_id IS NOT NULL)::int+(team_id IS NOT NULL)::int=1));
CREATE TABLE IF NOT EXISTS valorapesquisa.people_development_plan_items(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),development_plan_id uuid NOT NULL REFERENCES valorapesquisa.people_development_plans(id),description text NOT NULL,status varchar(20) NOT NULL DEFAULT 'open',due_date date,evidence_summary text,completed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),CHECK(status<>'completed' OR evidence_summary IS NOT NULL));
CREATE TABLE IF NOT EXISTS valorapesquisa.people_one_on_one_insights(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),profile_id uuid REFERENCES valorapesquisa.people_profiles(id),source_session_id uuid,insight_summary text NOT NULL,evidence_summary text NOT NULL,confidence varchar(16) NOT NULL DEFAULT 'low',access_classification varchar(20) NOT NULL DEFAULT 'sensitive',created_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.people_risk_signals(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),signal_type varchar(80) NOT NULL,evidence_summary text NOT NULL,confidence varchar(16) NOT NULL DEFAULT 'low',limitations text NOT NULL,status varchar(20) NOT NULL DEFAULT 'open',human_reviewed boolean NOT NULL DEFAULT false,review_notes text,reviewed_by_user_id uuid REFERENCES valorapesquisa.users(id),reviewed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS valorapesquisa.people_culture_events(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),event_type varchar(100) NOT NULL,entity_id uuid NOT NULL,performed_by_user_id uuid REFERENCES valorapesquisa.users(id),evidence_summary text NOT NULL,created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS valorapesquisa.people_privacy_consents(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),profile_id uuid NOT NULL REFERENCES valorapesquisa.people_profiles(id),purpose varchar(120) NOT NULL,status varchar(20) NOT NULL,granted_at timestamptz,revoked_at timestamptz,expires_at timestamptz,recorded_by_user_id uuid REFERENCES valorapesquisa.users(id),created_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_people_profiles_org ON valorapesquisa.people_profiles(organization_id,status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_people_teams_org ON valorapesquisa.people_teams(organization_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_people_culture_org_date ON valorapesquisa.people_culture_assessments(organization_id,assessed_at DESC) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_people_engagement_org_date ON valorapesquisa.people_engagement_signals(organization_id,observed_at DESC) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_people_risks_org ON valorapesquisa.people_risk_signals(organization_id,status,created_at DESC) WHERE deleted_at IS NULL;
INSERT INTO valorapesquisa.permissions(code,name,description,module_code) VALUES
('people.view','Consultar People','Consultar inteligência humana agregada e responsável.','organizational_intelligence'),('people.manage','Gerenciar People','Gerenciar perfis com proteção LGPD.','organizational_intelligence'),('people.teams.manage','Gerenciar times','Gerenciar vínculos e lideranças.','organizational_intelligence'),('people.culture.view','Consultar cultura','Consultar cultura agregada.','organizational_intelligence'),('people.culture.manage','Gerenciar cultura','Registrar avaliações culturais com evidência.','organizational_intelligence'),('people.engagement.view','Consultar engajamento','Consultar sinais agregados de engajamento.','organizational_intelligence'),('people.competencies.manage','Gerenciar competências','Avaliar competências com revisão humana.','organizational_intelligence'),('people.development_plans.manage','Gerenciar desenvolvimento','Gerenciar planos de desenvolvimento.','organizational_intelligence'),('people.risks.view','Consultar sinais humanos','Consultar sinais com limitações.','organizational_intelligence'),('people.risks.review','Revisar sinais humanos','Registrar revisão humana obrigatória.','organizational_intelligence')
ON CONFLICT(code) DO UPDATE SET name=excluded.name,description=excluded.description,module_code=excluded.module_code,updated_at=now();
...
-- Risk, Compliance & Internal Controls Center (idempotent, organization-scoped)
BEGIN;
SET LOCAL search_path TO valorapesquisa, public;
CREATE TABLE IF NOT EXISTS risk_registers (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),name text NOT NULL,status text NOT NULL DEFAULT 'active',owner_user_id uuid REFERENCES users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS risk_categories (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),name text NOT NULL,description text,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(organization_id,name));
CREATE TABLE IF NOT EXISTS organizational_risks (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),register_id uuid REFERENCES risk_registers(id),category_id uuid NOT NULL REFERENCES risk_categories(id),title text NOT NULL,description text,probability numeric(6,2) NOT NULL CHECK(probability BETWEEN 1 AND 5),impact numeric(6,2) NOT NULL CHECK(impact BETWEEN 1 AND 5),criticality numeric(10,2) NOT NULL,owner_user_id uuid NOT NULL REFERENCES users(id),status text NOT NULL CHECK(status IN('identified','assessed','mitigating','accepted','closed')),created_by_user_id uuid NOT NULL REFERENCES users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS risk_evidences (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),risk_id uuid NOT NULL REFERENCES organizational_risks(id),evidence_type text NOT NULL,title text NOT NULL,source_uri text,content_hash text,observed_at timestamptz,created_by_user_id uuid NOT NULL REFERENCES users(id),created_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS risk_assessments (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),risk_id uuid NOT NULL REFERENCES organizational_risks(id),probability numeric(6,2) NOT NULL,impact numeric(6,2) NOT NULL,base_score numeric(10,2) NOT NULL,evidence_adjustment numeric(6,4) NOT NULL,adjusted_score numeric(10,2) NOT NULL,confidence text NOT NULL CHECK(confidence IN('hypothesis','low','evidence_supported')),rationale text,assessed_by_user_id uuid NOT NULL REFERENCES users(id),assessed_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS compliance_frameworks (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),name text NOT NULL,version text NOT NULL,status text NOT NULL DEFAULT 'draft',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(organization_id,name,version));
CREATE TABLE IF NOT EXISTS compliance_requirements (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),framework_id uuid NOT NULL REFERENCES compliance_frameworks(id),code text NOT NULL,title text NOT NULL,description text,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,UNIQUE(framework_id,code));
CREATE TABLE IF NOT EXISTS risk_controls (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),risk_id uuid REFERENCES organizational_risks(id),requirement_id uuid REFERENCES compliance_requirements(id),name text NOT NULL,description text,state text NOT NULL CHECK(state IN('designed','implemented','tested','ineffective','retired')),owner_user_id uuid NOT NULL REFERENCES users(id),created_by_user_id uuid NOT NULL REFERENCES users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,CHECK((risk_id IS NOT NULL)::int+(requirement_id IS NOT NULL)::int=1));
CREATE TABLE IF NOT EXISTS control_tests (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),control_id uuid NOT NULL REFERENCES risk_controls(id),result text NOT NULL CHECK(result IN('effective','partially_effective','ineffective')),notes text,responsible_user_id uuid NOT NULL REFERENCES users(id),evidence_ids uuid[] NOT NULL CHECK(cardinality(evidence_ids)>0),tested_by_user_id uuid NOT NULL REFERENCES users(id),tested_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS compliance_assessments (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),requirement_id uuid NOT NULL REFERENCES compliance_requirements(id),result text NOT NULL CHECK(result IN('conformant','partially_conformant','non_conformant','not_assessed')),rationale text,evidence_ids uuid[] NOT NULL DEFAULT '{}',assessed_by_user_id uuid NOT NULL REFERENCES users(id),assessed_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,CHECK(result='not_assessed' OR cardinality(evidence_ids)>0));
CREATE TABLE IF NOT EXISTS non_conformities (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),requirement_id uuid REFERENCES compliance_requirements(id),title text NOT NULL,root_cause text,impact text,severity text NOT NULL CHECK(severity IN('low','medium','high','critical')),status text NOT NULL DEFAULT 'open',detected_by_user_id uuid NOT NULL REFERENCES users(id),detected_at timestamptz NOT NULL DEFAULT now(),closed_at timestamptz,deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS mitigation_plans (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),non_conformity_id uuid NOT NULL REFERENCES non_conformities(id),title text NOT NULL,responsible_user_id uuid NOT NULL REFERENCES users(id),due_at timestamptz NOT NULL,priority text NOT NULL CHECK(priority IN('low','medium','high','critical')),status text NOT NULL CHECK(status IN('planned','in_progress','blocked','completed')),completion_evidence_ids uuid[] NOT NULL DEFAULT '{}',created_by_user_id uuid NOT NULL REFERENCES users(id),created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,CHECK(status<>'completed' OR cardinality(completion_evidence_ids)>0));
CREATE TABLE IF NOT EXISTS mitigation_plan_actions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),mitigation_plan_id uuid NOT NULL REFERENCES mitigation_plans(id),title text NOT NULL,responsible_user_id uuid NOT NULL REFERENCES users(id),due_at timestamptz NOT NULL,status text NOT NULL DEFAULT 'pending',completion_evidence_ids uuid[] NOT NULL DEFAULT '{}',completed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz,CHECK(status<>'completed' OR cardinality(completion_evidence_ids)>0));
CREATE TABLE IF NOT EXISTS audit_reviews (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),title text NOT NULL,scope text NOT NULL,status text NOT NULL DEFAULT 'planned',lead_auditor_user_id uuid NOT NULL REFERENCES users(id),started_at timestamptz,completed_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS audit_findings (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),audit_review_id uuid NOT NULL REFERENCES audit_reviews(id),risk_id uuid REFERENCES organizational_risks(id),title text NOT NULL,description text,severity text NOT NULL,recommendation text,evidence_ids uuid[] NOT NULL DEFAULT '{}',status text NOT NULL DEFAULT 'open',created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now(),deleted_at timestamptz);
CREATE TABLE IF NOT EXISTS risk_events (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),entity_type text NOT NULL,entity_id uuid NOT NULL,event_type text NOT NULL,actor_user_id uuid NOT NULL REFERENCES users(id),summary text NOT NULL,metadata_json jsonb NOT NULL DEFAULT '{}',created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS risk_heatmap_snapshots (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),organization_id uuid NOT NULL REFERENCES organizations(id),snapshot_json jsonb NOT NULL,generated_by_user_id uuid NOT NULL REFERENCES users(id),generated_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS ix_organizational_risks_org_criticality ON organizational_risks(organization_id,criticality DESC) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_risk_evidences_risk ON risk_evidences(organization_id,risk_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_risk_controls_org_state ON risk_controls(organization_id,state) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_non_conformities_org_status ON non_conformities(organization_id,status) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_mitigation_plans_org_due ON mitigation_plans(organization_id,due_at) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS ix_risk_events_org_created ON risk_events(organization_id,created_at DESC);
INSERT INTO permissions(code,name,description,module_code) VALUES
('risk_compliance.view','Visualizar Risk & Compliance','Consultar riscos e conformidade baseados em evidências','organizational_intelligence'),
('risk_compliance.manage','Gerenciar Risk & Compliance','Administrar o centro de riscos e controles','organizational_intelligence'),
('risks.manage','Gerenciar riscos','Registrar e avaliar riscos organizacionais','organizational_intelligence'),
('controls.manage','Gerenciar controles','Administrar e testar controles internos','organizational_intelligence'),
('compliance.manage','Gerenciar conformidade','Avaliar requisitos com evidências','organizational_intelligence'),
('non_conformities.manage','Gerenciar não conformidades','Registrar e acompanhar não conformidades','organizational_intelligence'),
('mitigation_plans.manage','Gerenciar mitigação','Administrar planos e ações de mitigação','organizational_intelligence'),
('audits.manage','Gerenciar auditorias','Administrar revisões e achados','organizational_intelligence'),
...
-- ActionCenter: idempotência forte da criação direta de planos.
CREATE TABLE IF NOT EXISTS valorapesquisa.action_plan_create_commands(
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES valorapesquisa.organizations(id),
 created_by_user_id uuid NOT NULL REFERENCES valorapesquisa.users(id), command_id varchar(80) NOT NULL,
 operation varchar(40) NOT NULL, request_hash char(64) NOT NULL, result_id uuid NOT NULL REFERENCES valorapesquisa.action_plans(id),
 created_at timestamptz NOT NULL DEFAULT now(), UNIQUE(organization_id,created_by_user_id,operation,command_id));
