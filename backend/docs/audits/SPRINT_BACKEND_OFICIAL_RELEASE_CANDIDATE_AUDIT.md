...
## 1. Resumo
A sprint preparou o Release Candidate documental e operacional da versão oficial em `backend/Valora.sln`, sem evoluir `projeto .NET predecessor removido`, sem criar novo frontend e sem versionar secrets/dumps/dados reais. O ambiente atual bloqueou validações .NET/Docker reais por ausência de ferramentas.
