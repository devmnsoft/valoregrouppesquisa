...
public sealed class IntelligenceProcessingJobRepository(IDbConnectionFactory connections) : IIntelligenceProcessingJobRepository {
    ...
    public async Task<IReadOnlyList<IntelligenceProcessingJob>> ClaimPendingJobsAsync(int take, string workerId, CancellationToken ct) {
        var sql = $"""
            WITH candidates AS (
                SELECT id
                FROM valorapesquisa.intelligence_processing_jobs
                WHERE deleted_at IS NULL
                  AND status IN ('pending','retry_scheduled')
                  AND coalesce(scheduled_at,created_at) <= now()
                  AND coalesce(next_attempt_at,now()) <= now()
                  AND (locked_at IS NULL OR locked_at < now() - interval '10 minutes')
                ORDER BY priority,coalesce(scheduled_at,created_at)
                FOR UPDATE SKIP LOCKED
                LIMIT @take
            ), claimed AS (
                UPDATE valorapesquisa.intelligence_processing_jobs AS job
        ...
        var rows = await db.QueryAsync<IntelligenceProcessingJobRow>(new CommandDefinition(sql, new { take, workerId }, cancellationToken: ct));
    ...
    public Task MarkFailedAsync(Guid id, string code, string message, CancellationToken ct) => Execute("UPDATE valorapesquisa.intelligence_processing_jobs SET status='dead_letter',failed_at=now(),locked_at=NULL,locked_by=NULL,error_code=@code,error_message=@message,updated_at=now() WHERE id=@id", new { id, code, message }, ct);
    ...
    private sealed class StageRow {
        ...
        public Guid RunId { get; set; }
        public string Stage { get; set; } = string.Empty; public string Status { get; set; } = string.Empty;
