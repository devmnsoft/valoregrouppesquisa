...
public sealed class MigrationController(
    ...
    IMigrationReportService reports, IAuditRepository audit) : ControllerBase
{
    ...
    [HttpGet("/admin/migration/status")] public IActionResult Status() => Ok(new { ok = true, dataProvider = "api-postgresql-local", postgresSchema = "valorapesquisa", legado = "preserved", sensitivePayload = "masked" });
    [HttpPost("/migration/sources")] public IActionResult RegisterSource([FromBody] MigrationUploadRequest r)=>Safe(async()=> { var created = await sources.CreateAsync(null,r.FileName,r.ContentType,System.Text.Encoding.UTF8.GetByteCount(r.PayloadJson),Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(System.Text.Encoding.UTF8.GetBytes(r.PayloadJson))).ToLowerInvariant(),null,"registered"); await audit.AddAsync(new AuditEntry(null,null,"migration.source.registered","migration_source",created.Id.ToString(),"Source registrado sem payload sensível","{}")); return created; });
    ...
    [HttpPost("/migration/batches")] public IActionResult CreateBatch([FromBody] MigrationUploadRequest r)=>Safe(async()=>{ var b=await batches.CreateAsync(r.SourceType,r.SourceName,"dry_run",User.Identity?.Name??"admin_valora"); await audit.AddAsync(new AuditEntry(null,null,"migration.batch.created","migration_batch",b.Id.ToString(),"Batch criado","{}")); return b; });
    ...
    [HttpPatch("/migration/conflicts/{conflictId:guid}/resolution")] public IActionResult Resolve(Guid conflictId,[FromBody] Dictionary<string,string> body)=>Safe(async()=>{await conflicts.ResolveAsync(conflictId,body.GetValueOrDefault("resolution")??"manual_review",User.Identity?.Name??"admin_valora"); await audit.AddAsync(new AuditEntry(null,null,"migration.conflict.resolved","migration_conflict",conflictId.ToString(),"Conflito resolvido","{}")); return new{resolved=true};});
