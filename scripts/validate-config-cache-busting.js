...
assert(/APP_VERSION:\s*'8.7.6'/.test(config),'APP_VERSION 8.7.6');
['config.js','app.js','style.css'].forEach(a=>assert(index.includes(`${a}?v=8.7.6`),`${a} versionado no index`));
...
assert(/config\.js\?v=8.7.6/.test(build),'dist referencia config.js versionado');
finish();
