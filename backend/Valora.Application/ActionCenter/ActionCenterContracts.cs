...
public interface IActionPlanRepository { Task<ActionDashboardDto> Dashboard(Guid organizationId, CancellationToken ct); Task<IReadOnlyList<ActionPlanDto>> List(Guid organizationId, CancellationToken ct); Task<PageResult<ActionOptionDto>> Options(Guid organizationId, Guid userId, bool organizationWide, OptionQuery query, CancellationToken ct); Task<ActionPlanDto?> Get(Guid organizationId, Guid id, CancellationToken ct); Task<Guid> Create(Guid organizationId, Guid userId, CreateActionPlanRequest request, CancellationToken ct); }
public interface IActionItemRepository { Task<IReadOnlyList<ActionItemDto>> List(Guid organizationId, Guid? planId, CancellationToken ct); Task<PageResult<ActionOptionDto>> Options(Guid organizationId, Guid userId, bool organizationWide, OptionQuery query, CancellationToken ct); Task<ActionItemDto?> Get(Guid organizationId, Guid id, CancellationToken ct); Task<Guid> Create(Guid organizationId, Guid userId, CreateActionItemRequest request, CancellationToken ct); Task UpdateProgress(Guid organizationId, Guid userId, Guid id, int progress, string? note, CancellationToken ct); Task Block(Guid organizationId, Guid userId, Guid id, string reason, CancellationToken ct); Task Complete(Guid organizationId, Guid userId, Guid id, string result, string evidence, CancellationToken ct); }
