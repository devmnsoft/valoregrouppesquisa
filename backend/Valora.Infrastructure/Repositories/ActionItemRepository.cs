...
public sealed class ActionItemRepository(IDbConnectionFactory db, IDbTransactionFactory tx) : IActionItemRepository {
    ...
    public async Task<PageResult<ActionItemDto>> List(Guid o, Guid u, Guid? planId, bool wide, ActionItemListQuery query, CancellationToken c) {
        ...
        var where=Scope+" AND (@planId IS NULL OR i.action_plan_id=@planId) AND (@status IS NULL OR i.status=@status) AND (@open=false OR i.status NOT IN('completed','canceled')) AND (@priority IS NULL OR i.priority=@priority) AND (@responsible IS NULL OR i.responsible_user_id=@responsible) AND (@assignment IS NULL OR @assignment='Assigned' AND i.responsible_user_id IS NOT NULL OR @assignment='Unassigned' AND i.responsible_user_id IS NULL) AND (@search IS NULL OR i.title ILIKE '%'||@search||'%' OR i.description ILIKE '%'||@search||'%' OR p.title ILIKE '%'||@search||'%') AND (@due IS NULL OR (@due='overdue' AND (i.due_at AT TIME ZONE org.time_zone)::date<(now() AT TIME ZONE org.time_zone)::date AND i.status NOT IN ('completed','canceled')) OR (@due='today' AND (i.due_at AT TIME ZONE org.time_zone)::date=(now() AT TIME ZONE org.time_zone)::date) OR (@due='upcoming' AND (i.due_at AT TIME ZONE org.time_zone)::date>(now() AT TIME ZONE org.time_zone)::date) OR (@due='none' AND i.due_at IS NULL))";
        using var x=db.Create(); using var q=await x.QueryMultipleAsync(new CommandDefinition($"SELECT count(*)::int FROM valorapesquisa.action_items i JOIN valorapesquisa.action_plans p ON p.id=i.action_plan_id AND p.organization_id=i.organization_id JOIN valorapesquisa.organizations org ON org.id=i.organization_id WHERE {where}; {Projection} JOIN valorapesquisa.organizations org ON org.id=i.organization_id WHERE {where} ORDER BY i.due_at NULLS LAST,i.created_at DESC,i.id LIMIT @pageSize OFFSET @offset",new{o,u,planId,wide,status,open=query.Scope==ActionItemScopeFilter.Open,priority,responsible=query.ResponsibleUserId,assignment=query.Assignment?.ToString(),search,due,pageSize,offset},cancellationToken:c));
    ...
    public async Task<ActionItemDetailsDto?> Details(Guid o, Guid u, Guid id, bool wide, bool canManage, bool canComplete, int historyPage, CancellationToken c) {
        ...
        const string sql = """
            ...
            SELECT h.id,h.from_status FromStatus,h.to_status ToStatus,h.progress_percent ProgressPercent,h.reason,u.name AuthorName,h.changed_at ChangedAt,h.operation,h.from_value FromValue,h.to_value ToValue,h.intent_version IntentVersion
        ...
        return new(item, history, page, Math.Max(1, (count + size - 1) / size), canManage&&operational, canManage&&operational, canManage&&operational, canManage&&operational, canManage&&operational, canManage&&item.Status=="blocked", canComplete&&operational);
    }
    ...
    public async Task<Guid> Create(Guid o, Guid u, CreateActionItemRequest r, CancellationToken c) {
        ...
        await z.Connection.ExecuteAsync(new CommandDefinition("SELECT id FROM valorapesquisa.action_plans WHERE id=@PlanId AND organization_id=@o FOR UPDATE",new{o,normalized.PlanId},z.Transaction,cancellationToken:c));
        var id=Guid.NewGuid();
    ...
    public Task Cancel(Guid o,Guid u,Guid id,bool wide,TransitionActionRequest r,CancellationToken c)=>Transition(o,u,id,wide,r.Version,r.CommandId,"cancel",new[]{"pending","in_progress","blocked","overdue"},"canceled",null,r.Reason,null,null,c);
    ...
    public Task Edit(Guid o,Guid u,Guid id,bool wide,EditActionItemRequest r,CancellationToken c) => Modify(o,u,id,wide,r.Version,r.CommandId,"edit",r, null,c);
    ...
    private async Task Modify(Guid o,Guid u,Guid id,bool wide,long version,string commandId,string operation,object request,string? reason,CancellationToken c) {
        if(string.IsNullOrWhiteSpace(commandId)) throw new ArgumentException("A chave da operação é obrigatória.");
        await using var z=await tx.BeginAsync(c);
        var intent=CreateFingerprint(o,u,operation,new{version,request});
        ...
        await z.Connection.ExecuteAsync(new CommandDefinition("SELECT p.id FROM valorapesquisa.action_plans p JOIN valorapesquisa.action_items i ON i.action_plan_id=p.id AND i.organization_id=p.organization_id WHERE i.id=@id AND i.organization_id=@o FOR UPDATE OF p",new{o,id},z.Transaction,cancellationToken:c));
        var current=await z.Connection.QuerySingleOrDefaultAsync<EditableItem>(new CommandDefinition("SELECT i.status,i.version,i.title,i.description,i.priority,i.expected_outcome ExpectedOutcome,i.responsible_user_id ResponsibleUserId,(i.due_at AT TIME ZONE org.time_zone)::date DueAt,old_user.name ResponsibleName FROM valorapesquisa.action_items i JOIN valorapesquisa.organizations org ON org.id=i.organization_id JOIN valorapesquisa.action_plans p ON p.id=i.action_plan_id AND p.organization_id=i.organization_id LEFT JOIN valorapesquisa.users old_user ON old_user.id=i.responsible_user_id AND old_user.organization_id=i.organization_id WHERE i.id=@id AND i.organization_id=@o AND i.deleted_at IS NULL AND p.deleted_at IS NULL AND (@wide OR i.responsible_user_id IS NULL OR i.responsible_user_id=@u OR p.owner_user_id=@u) FOR UPDATE OF i",new{o,u,id,wide},z.Transaction,cancellationToken:c));
        if(current is null) throw new KeyNotFoundException("Atividade não encontrada no escopo autorizado.");
        var replay=await z.Connection.QuerySingleOrDefaultAsync<ReplayState>(new CommandDefinition("SELECT operation,intent_hash IntentHash,intent_version IntentVersion,changed_by_user_id ChangedBy FROM valorapesquisa.action_item_status_history WHERE action_item_id=@id AND command_id=@commandId",new{id,commandId},z.Transaction,cancellationToken:c));
        if(replay is not null){if(replay.Operation==operation&&replay.IntentHash==intent&&replay.IntentVersion==version&&replay.ChangedBy==u){await z.CommitAsync();return;}throw new ConcurrencyConflictException("A chave da operação já foi usada com outra intenção.");}
        if(current.Version!=version) throw new ConcurrencyConflictException("A atividade foi alterada. Consulte os valores atuais antes de reenviar.");
        if(current.Status is "completed" or "canceled") throw new ConflictAppException("Atividades concluídas ou canceladas não podem ser editadas.");
        string before;string after;int changed;
        if(request is EditActionItemRequest edit){before=System.Text.Json.JsonSerializer.Serialize(new{current.Title,current.Description,current.Priority,current.ExpectedOutcome});after=System.Text.Json.JsonSerializer.Serialize(new{Title=edit.Title.Trim(),Description=edit.Description.Trim(),Priority=edit.Priority,ExpectedOutcome=edit.ExpectedOutcome.Trim()});if(before==after)throw new ConflictAppException("Nenhuma alteração foi informada.");changed=await z.Connection.ExecuteAsync(new CommandDefinition("UPDATE valorapesquisa.action_items SET title=@Title,description=@Description,priority=@Priority,expected_outcome=@ExpectedOutcome,version=version+1,updated_at=now() WHERE id=@id AND organization_id=@o AND version=@version",new{o,id,version,Title=edit.Title.Trim(),Description=edit.Description.Trim(),edit.Priority,ExpectedOutcome=edit.ExpectedOutcome.Trim()},z.Transaction,cancellationToken:c));}
        else if(request is AssignResponsibleRequest assign){if(current.ResponsibleUserId.HasValue&&string.IsNullOrWhiteSpace(assign.Reason))throw new ArgumentException("Justifique a substituição ou remoção do responsável atual.");var next=assign.ResponsibleUserId.HasValue?await z.Connection.QuerySingleOrDefaultAsync<string>(new CommandDefinition("SELECT name FROM valorapesquisa.users WHERE id=@responsible AND organization_id=@o AND status='active' AND deleted_at IS NULL",new{o,responsible=assign.ResponsibleUserId},z.Transaction,cancellationToken:c)):null;if(assign.ResponsibleUserId.HasValue&&next is null)throw new ConflictAppException("A pessoa selecionada não está ativa e elegível nesta organização.");if(current.ResponsibleUserId==assign.ResponsibleUserId)throw new ConflictAppException("O responsável selecionado já é o responsável atual.");before=current.ResponsibleName??"Sem responsável";after=next??"Sem responsável";changed=await z.Connection.ExecuteAsync(new CommandDefinition("UPDATE valorapesquisa.action_items SET responsible_user_id=@ResponsibleUserId,version=version+1,updated_at=now() WHERE id=@id AND organization_id=@o AND version=@version",new{o,id,version,assign.ResponsibleUserId},z.Transaction,cancellationToken:c));}
        else {var due=(RescheduleActionRequest)request;if(due.DueAt is null)throw new ArgumentException("Informe o novo prazo.");before=current.DueAt?.ToString("yyyy-MM-dd")??"Sem prazo";after=due.DueAt.Value.ToString("yyyy-MM-dd");if(before==after)throw new ConflictAppException("O novo prazo é igual ao prazo atual.");changed=await z.Connection.ExecuteAsync(new CommandDefinition("UPDATE valorapesquisa.action_items i SET due_at=((@DueAt::date+1)::timestamp AT TIME ZONE org.time_zone)-interval '1 microsecond',version=version+1,updated_at=now() FROM valorapesquisa.organizations org WHERE i.id=@id AND i.organization_id=@o AND org.id=@o AND i.version=@version",new{o,id,version,due.DueAt},z.Transaction,cancellationToken:c));}
        if(changed!=1)throw new ConcurrencyConflictException("A atividade foi alterada durante a operação.");
        await z.Connection.ExecuteAsync(new CommandDefinition("INSERT INTO valorapesquisa.action_item_status_history(action_item_id,from_status,to_status,reason,changed_by_user_id,command_id,operation,intent_hash,intent_version,from_value,to_value) VALUES(@id,@status,@status,@reason,@u,@commandId,@operation,@intent,@version,@before,@after)",new{id,status=current.Status,reason=reason?.Trim(),u,commandId,operation,intent,version,before,after},z.Transaction,cancellationToken:c));
        await z.CommitAsync();
    }
    ...
    private async Task Transition(Guid o, Guid u, Guid id, bool wide, long version, string commandId, string operation, string[] allowed, string next, int? progress, string? reason, string? result, string? evidence, CancellationToken c) {
        ...
        await z.Connection.ExecuteAsync(new CommandDefinition("SELECT pg_advisory_xact_lock(hashtextextended(@lockKey,0))",new{lockKey=$"{o:N}:{id:N}:{commandId}"},z.Transaction,cancellationToken:c));
        await z.Connection.ExecuteAsync(new CommandDefinition("SELECT p.id FROM valorapesquisa.action_plans p JOIN valorapesquisa.action_items i ON i.action_plan_id=p.id AND i.organization_id=p.organization_id WHERE i.id=@id AND i.organization_id=@o FOR UPDATE OF p",new{o,id},z.Transaction,cancellationToken:c));
        var current = await z.Connection.QuerySingleOrDefaultAsync<ItemState>(new CommandDefinition("SELECT i.status,i.version,i.title,p.status PlanStatus FROM valorapesquisa.action_items i JOIN valorapesquisa.action_plans p ON p.id=i.action_plan_id AND p.organization_id=i.organization_id WHERE i.id=@id AND i.organization_id=@o AND i.deleted_at IS NULL AND p.deleted_at IS NULL AND (@wide OR i.responsible_user_id IS NULL OR i.responsible_user_id=@u OR p.owner_user_id=@u) FOR UPDATE OF i",new{o,u,id,wide},z.Transaction,cancellationToken:c));
        if (current is null) throw new KeyNotFoundException("Atividade não encontrada no escopo autorizado.");
        ...
        if(operation!="cancel"&&current.PlanStatus!="in_execution") throw new ConflictAppException("A atividade foi preservada, mas sua execução exige que o plano seja aprovado e iniciado. Regularize o ciclo de vida do plano antes de continuar.");
    ...
    private sealed record ItemState(string Status,long Version,string Title,string PlanStatus);
    private sealed record EditableItem(string Status,long Version,string Title,string Description,string Priority,string ExpectedOutcome,Guid? ResponsibleUserId,DateTime? DueAt,string? ResponsibleName);
