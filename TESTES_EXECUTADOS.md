...
## Bootstrap inicial PRD Firebase — 2026-06-22

### Escopo validado em código

- Criado `scripts/bootstrap-firebase-prd.js` com `--dry-run`, `--apply`, `--merge`, proteção para `--overwrite`, criação idempotente de Admin Valora, Auth, custom claims, planos, módulos, settings, organização, formulário, pesquisa ativa, base de conhecimento, categorias de atendimento, SLA e resposta demo opcional.
- Criado `scripts/validate-prd-bootstrap.js` para validar consistência mínima de Auth/Firestore antes do primeiro acesso PRD.
- Confirmado que `getEmailStatus` é callable (`onCall`) e que o frontend usa `httpsCallable('getEmailStatus')`, evitando `fetch` direto para `cloudfunctions.net/getEmailStatus`.

### Comandos executados nesta entrega

| Comando | Resultado | Observação |
| --- | --- | --- |
| `node --check scripts/bootstrap-firebase-prd.js` | OK | Validação sintática do bootstrap. |
| `node --check scripts/validate-prd-bootstrap.js` | OK | Validação sintática do validador. |
| `node scripts/bootstrap-firebase-prd.js --project gestordepesquisa --dry-run` | OK | Dry-run sem escrita; lista documentos planejados e informa link público gerado uma única vez. |
| `npm run check` | OK | Validação sintática geral do projeto. |

### Checklist para execução real em PRD

- Dry-run: `node scripts/bootstrap-firebase-prd.js --project gestordepesquisa --dry-run` deve rodar sem escrever e mostrar o que será criado/mesclado.
