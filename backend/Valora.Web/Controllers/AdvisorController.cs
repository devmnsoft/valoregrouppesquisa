...
public sealed class AdvisorController(ICurrentOrganizationProvider current,AdvisorConversationService conversations,AdvisorMessageService messages,AdvisorContextBuilderService context,AdvisorPromptTemplateService templates):Controller
{
    [HttpGet] public async Task<IActionResult> Index(Guid? id,CancellationToken ct)
    {
        ...
        var list=await conversations.List(scope.OrganizationId,UserId(),ct);var active=id.HasValue?await conversations.Get(scope.OrganizationId,UserId(),id.Value,ct):null;
        return View(new AdvisorIndexViewModel{Conversations=list,ActiveConversation=active,ContextOptions=await context.Options(scope.OrganizationId,ct),Ask=new(){ConversationId=id}});
    }
    [HttpPost,ValidateAntiForgeryToken] public async Task<IActionResult> Ask(AdvisorAskForm form,CancellationToken ct)
    {
        ...
        try
        {
            var id=form.ConversationId??await conversations.Create(scope.OrganizationId,UserId(),new(){Objective=form.Objective},ct);
            var selected=form.SourceKeys.Select(ParseSource).Where(x=>x is not null).Cast<AdvisorContextSelection>().ToArray();
            await messages.Send(scope.OrganizationId,UserId(),id,new(){Content=form.Question,Context=selected},ct);return RedirectToAction(nameof(Index),new{id});
        }
    ...
    private async Task<AdvisorListPageViewModel> Page(string title,string description,CancellationToken ct){var o=current.GetCurrent();if(!o.IsResolved)return new(title,description,[],[]);return new(title,description,await conversations.List(o.OrganizationId,UserId(),ct),await templates.List(o.OrganizationId,ct));}
