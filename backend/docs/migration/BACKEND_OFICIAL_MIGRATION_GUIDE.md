...
- Banco oficial: PostgreSQL/Dapper no schema `valorapesquisa`, com scripts em `backend/database/postgresql`.

## Papel do `projeto .NET predecessor removido`

`projeto .NET predecessor removido` é apenas referência temporária. Ele não deve receber novas evoluções, não deve ser usado como base de build oficial e não deve gerar outro frontend ou solution paralela.
...
- Não apagar `projeto .NET predecessor removido` ainda.
...
Esta documentação passa a considerar o Release Candidate `0.9.0-rc1` como pacote de homologação real da versão oficial localizada em `backend/Valora.sln` e `backend/database/postgresql`. O legado e `projeto .NET predecessor removido` permanecem apenas como referência histórica e não fazem parte do build oficial.
...
- Base oficial mantida em `backend/Valora.sln`; `projeto .NET predecessor removido` segue apenas como referência histórica e o legado da raiz permanece preservado.
