...
public sealed class ResponseRepository(IDbConnectionFactory f, ILogger<ResponseRepository> logger) : IResponseRepository {
    ...
    public async Task<AdminResultReadModel?> GetAdminAsync(Guid organizationId, Guid responseId) { try { using var c = f.Create(); const string sql="""
